...
> **builtins 的两面性（易烧脑，先立此存照）**：builtins 同时携带 `.sc` 面
> （模块接口/实现的 sc 源）与 C 面（`platform.h`、`op_impl.c`、模块 `.c/.m`
> 源或预编 `.a`），被流水线的**两个不同阶段**分别消费——
> **转译期**（sc → C 文本，含 `--emit-c`）只解析 `.sc` 面（`inc` 的模块要能
> 被找到做语义分析），完全不碰 C 面、不需要 C 工具链；
> **C 编译期**（cc 编译生成的 C）只消费 C 面（生成的 C 统一
> `#include "platform.h"`，手动编译须 `-I builtins`，§3.6 的 emit-c 示例）。
> 因此"scc 转译不依赖 builtins 的库实现，但编译其产物依赖"——两阶段可以
> 分离在不同机器上发生（这正是远程构建 §6 与交叉编译 §5 的结构基础）。
> 构建 scc 二进制本身则两面都不需要（纯 C++，§7.9 依赖链的起点）。
