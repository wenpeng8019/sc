...
重算。一组 token 经 `dep…map` 连成**显式依赖图**（工作流 / 流水线 / 神经网络），编译器在**编译期**
...
`form t, v` 是语言内置语句（与 `run` / `done` 同级）：给 form token 灌初值（`@`）并升格为 form 主，
回放 form 前挂起的 `set`，触发就绪依赖。
