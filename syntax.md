...
> 编译器展开 `mix` 时即把二者登记进 sc，可直接把 `&ARGS_DEF_abc` 传给 `ARGS_parse(...)`。
...
| 写法 | 用途 | 形态 | 类型检查 |
| --- | --- | --- | --- |
| `def NAME: = v` / `def name: p,…` + `mix` | 结构化宏（对象宏 / 函数宏） | 生成 `#define`，`mix` 展开调用 | 否（文本展开） |
...
def DEFINE_COUNTER:: N
    ...
    fnc counter_\N\_inc:: void  # 该宏展开出的函数原型
...
- **(a) 文本宏路径**（`macroTypeParams` 空、`cImpl` 关 —— §18.1 / §18.3）：
...
- **(b) 单态化路径**（`macroTypeParams` 非空 —— §18.4）：
  在「解析之后、语义检查之前」插入一趟 `expandGenericMixes` 单态化 pass。它对每个 `mix` 实例：
  **克隆宏体 AST → 把类型参数替换成具体类型 → 重新 emit 成 sc 文本 → 再 parse 回具体声明**，
  于是得到一份份**真实的、具体类型的** `def`/`fnc` 节点，接着走**完整的 `semanticCheck` 与普通
  C 代码生成**。泛型 `def` 自身不再生成 `#define`（代码生成里 `if (!macroTypeParams.empty()) break;`
  跳过），已被消费的 `mix` 也不输出任何东西（`macroConsumed` 标记）。这就是它能做到「`i4` 会翻译、
  全量类型检查、错误落在源码」的原因——本质上它把泛型**降解成了普通 sc 代码**再编译。
...
flowchart TD
    ...
    B --> C{"带 &lt;T&gt; ?<br/>(macroTypeParams 非空)"}
    C -- "是" --> D["单态化 pass：克隆体→替换类型→重解析<br/>→ 具体声明 → 完整语义检查 + 普通 C 代码生成<br/>（不生成 #define）"]
    C -- "否" --> E{"名字带 :: ?<br/>(cImpl)"}
