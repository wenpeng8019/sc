...
- 自动指针 `T@`（与裸指针 `T&` 独立的 opt-in 路径，24 字节胖指针 `{p, tar, own}`）：`T()` 带头堆分配、双向边引用图（`in`/`out` 双计数）、域退出两阶段拆解、孤立对象 `in→0` ARC 自动 `free`、`out>0` 未清理报错、`&` 取址传染性与栈变量借用；`--check=ref`（或 `SCC_REF_CHECK`）开启带源码定位的悬挂诊断，默认关闭仅保留 ARC（详见 §4.1 与 builtins/auto_ptr.md）
