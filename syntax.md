...
>
> 远期规划支持 `--cstd=c99|c11|c17` 显式选择输出档位（见 §24），以适配老旧或受限工具链。
...
- `tst`：定义单元测试用例（块内用 `assert` 断言，`scc --test` 运行，见 §22）。
- `mix`：展开一个 `def` 宏（顶层展开声明，函数体内展开语句，见 §18.1「结构化宏」）。
- `inc`：引入头文件或 sc 模块。
- `add`：把实现文件（`.c`）、库文件（`.a`/`.so`/`.o`）或 sc 子单元（`.sc`）加入本单元；
  前两者参与编译/链接，`.sc` 则内联（源码拼接）进本单元（见 §7）。
...
- 任意对象 `in→0` → 若其类型有 `drop`，**先触发析构**（由 `drop` 经 `this->m = nil` 清理子成员出边），
  析构后 `out==0` → **ARC 自动 free**；`out>0`（漏清或无 `drop`）→ 报“未清理”错误。
  级联回收由 `drop` 的解绑驱动（子目标 `in--` 逐层触发各自 `drop`），ARC 本身不主动级联。
  析构体内经裸接收者 `this` 可**读**胖成员、可 **`= nil`** 解绑，但不可绑「新边」（须经 `T@` base）。
...
- **`T@` 数组**：支持**局部数组**（一维与多维，`var arr[N]: node@` / `var grid[M][N]: node@`）与
  **全局数组**（一维与多维），全局标量 `T@` 亦支持——声明零初始化，下标赋值 `arr[i] = node()`
  带头分配并绑根边；局部数组在退域/`return`/`break` 处、全局数组在进程退出（`destructor` 钩子
  `__sc_gfat_fini`）处逐元素 `unbind` 清理整张引用图（`in→0` 触发 `drop`，释放其持有的系统资源）；
  元素可借用、可取成员出边（`arr[i]->child = ...`）。**结构体字段/函数参数/返回值/`tls`** 位置的
  `T@` 数组仍未实现引用图记账 → 报错，请改用裸指针 `T&` 数组。（`T@` 数组元素的嵌套出边与标量
  同理，须在释放前显式清理或置 `nil`。）
- 含 `T@` 成员的结构体**不得跨 C ABI**（被 C 侧 memcpy/fread 会让内部指针成垃圾地址）。
- **禁止按值拷贝内嵌 `T@` 的聚合**：若结构体（递归）含 `T@` 成员，则整体 `=` 赋值、以初值
  拷贝构造（`var g: Holder = h`）、按值传参、按值返回均**编译报错**。
  *原因*：`T@` 是 24 字节胖指针 `{p, tar, own}`，按值拷贝会把这三字段当裸字节复制，绕过 `in`/`out`
  记账——两个副本指向同一目标却只记一次入边，退域时双重 `unbind` → 目标被提前/二次释放，或 `drop`
  重复触发。*改用*：以「无初值 + `init` 钩子」就地构造（`var g: Holder`），或经 `T@` / `&` 引用共享。
- 自动指针目标须**地址稳定**：不得把按值塞进可扩容容器的对象当作目标。
- `T@ → T&` 显式转裸（`r: node& = p`，取 `p.p`）是唯一的 opt-out 出口，转裸后不再追踪。

### 5.2 类型擦除自动指针 `object@` / 裸 `@` 与三者互转

`T@` 之外还有两种**类型擦除**的自动指针形态，三者参与同一套引用图记账（`in`/`out` 双计数、
释放点验证、ARC 自动 `free`），只在「编译期是否保留具体类型」上不同：

| 形态 | 编译期类型 | 能力 | 典型用途 |
|---|---|---|---|
| `T@` | 具体类型 `T` | 直接读写成员、调方法 | 日常持有一个已知类型的对象 |
| `object@` | 仅知「是个类」 | 经维度 `dim` 动态分派、`instanceOf` | 多态：持有任意 `cls`，按维度发消息 |
| 裸 `@` | 完全擦除 | 仅记账与析构，取出须显式还原 | 通用容器（`list`/`dict` 元素），装「某个自动指针」 |

```sc
var d: Dog@   = Dog()       # 具体类型
var o: object@ = d           # 升为多态句柄（Dog 是 cls）
var e: @       = d           # 擦成裸 @（通用容器持有）
(e: Dog@)->bark()            # 取出时显式还原类型再用
```

**核心约定——擦除随手可做，还原须自己点名。** 把任意形态升/擦成更模糊的形态（`T@ → object@`、
`T@ → @`、`object@ → @`）总是安全的，编译器自动接管；反向**还原**（`@ → T@`、`@ → object@`、
`object@ → T@`）是你对「里面装的到底是什么」的断言，写法上必须用强转 `(x: 目标形态)` 显式点名，
语义等同 C 里的向下转型——点错了由你负责（`--check=ref` 会在能判别时拦截，见下）。

**转换全景（经裸 `@` 中转的「过桥」四个方向都无损）。** 关键区别只有一条：

> **载荷是不是 `cls`（类）？** 是 → 擦成裸 `@` 时自动保留多态分派能力，四向过桥全部可还原；
> 否 → 普通 `@def`/`def` 结构体没有类分派能力，擦成裸 `@` 后**不能**再还原成 `object@`。

| 转换 | 是否支持 | 说明 |
|---|---|---|
| `T@ → object@` | ✅（`T` 须为 `cls`） | 升为多态句柄，可发维度消息 |
| `object@ → T@` | ✅ | 向下还原到具体类，自动校正类布局偏移 |
| `T@ → @` / `object@ → @` | ✅ | 擦成裸句柄交给容器；析构能力随句柄走 |
| `@ → T@` | ✅ | 显式还原具体类型后读写 |
| `@ → object@` | ✅（载荷须为 `cls`） | 还原多态句柄；非类载荷会被 `--check=ref` 拦截 |
| `T@ → @ → object@` | ✅（`T` 为 `cls`） | 过桥：`cls` 经裸 `@` 中转仍可升多态 |
| `T@ → @ → T@` / `object@ → @ → object@` | ✅ | 同形态往返 |
| 普通结构体 `@def T → @ → object@` | ❌ | 非类无分派槽，`object@` 对它无意义 |

```sc
@def node: { v: i4 }          # 普通结构体（非 cls）

var t: node@ = node()
var b: @ = t                 # 擦除 OK
var x: node@ = (b: node@)    # 还原具体类型 OK
# var bad: object@ = (b: object@)   # ❌ node 非 cls，开 --check=ref 会运行时 abort
```

**忘了里面装什么也能安全销毁。** 裸 `@` 在擦除点就把目标的析构方式记在句柄里，
容器退场、元素 `in→0` 时即便完全不知道具体类型也能正确触发 `drop` 并回收——这正是
通用容器持有 `@` 的前提。完整实现机制（句柄布局、风味判别、偏移校正）见
[builtins/auto_ptr.md](builtins/auto_ptr.md) §3.4，类机制（`cls`/`dim`/`object`）见 §19。

### 5.3 单例指针 `T@1`（裸指针 + 退域自动释放）

`T@1` 是介于裸指针 `T&` 与自动指针 `T@` 之间的第三条路径：**物理上就是裸指针 `T*`**
（8 字节，取值即裸地址，不参与引用图、无 `in`/`out` 记账），但额外带一条 **RAII 析构**——
退出作用域或被重新赋值覆盖旧值时，若它指向的对象非 `nil`，自动执行 `drop` + `free` 销毁。
语义等同 C++ 的 `unique_ptr`：**独占持有、随域释放**。动机是消除「拿到一个堆对象、用完手写
一长串 `->drop()`」的样板。

```sc
var raw: tensor&     # 裸指针：纯 C，谁分配谁释放（手写 drop）
var p:   tensor@1    # 单例指针：裸地址 + 退域自动销毁
var a:   tensor@     # 自动指针：24 字节胖指针，参与引用图共享
```

**赋值来源受限（独占语义）。** `T@1` 只接受三类右值：取址 `&x`、`nil`、或**堆构造/工厂/方法**
返回的新对象（如 `zeros(...)`、`p->reshape(...)`、数组取出 `(out[0]: tensor&)`）。它不做引用图
共享——把同一个对象同时塞进两个 `T@1` 会在退域时双重释放，由你负责保证每个 `T@1` 独占一个对象。

```sc
@fnc main: i4
    var p: tensor@1 = zeros(2, shp, DT_F4)   # 持有一个新箱子
    p->fill(1.0)
    p = ones(2, shp, DT_F4)                  # 重新赋值：先销毁旧箱子，再持有新的
    var r: tensor& = p                        # 取裸地址借用（不转移所有权、不销毁）
    return 0                                   # 退域：p 非 nil → 自动 drop + free
```

- **退域销毁**：作用域结束时逆序对每个非 `nil` 的 `T@1` 发安全析构；析构前先把指针槽置为
  `nil`，再执行 `drop`（若有）和 `free`。显式 `p->drop()` 后仍保留退域登记，但因为 `p`
  已被置为 `nil`，退域路径自动成为空操作；即使显式析构位于未执行的条件分支，仍不会泄漏。
- **重赋销旧**：`p = 新值` 先把新值算进临时，再销毁 `p` 原指对象，最后写入（自赋值安全）。
- **`= nil`** 即提前销毁并置空。
- **`return p`** 是**移动**：跳过该变量的退域销毁，所有权转交调用方。
- **取裸借用**：`var r: T& = p` / 强转 `(p: T&)` 取出裸地址供只读/传参，**不**销毁、**不**转移；
  注意借用别名 `var q: T& = p` 必须保持裸 `&`（不能再标 `@1`），否则双重释放。
- **显式堆指针析构**：对 `T&` 或 `T@1` 的可寻址接收者调用 `p->drop()`（兼容 `p.drop()`）时，
  编译器生成 nil 守卫，并在调用用户 `drop` 前将 `p` 置为 `nil`，随后回收外层对象块。对已为
  `nil` 的指针或已经析构过的同一指针重复调用，均为空操作。临时返回值等不可寻址接收者只有
  nil 守卫，无法把源表达式置空，不应保存后重复使用。
- **值对象的边界**：普通值对象 `T` 的 `x.drop()` 仍由类型自身负责重置状态；编译器会因显式
  调用而跳过该变量的退域自动 `drop`，但不把任意用户自定义 `drop` 宣布为运行时幂等。结构体
  字段级自动析构和更精细的控制流所有权分析不属于当前规则。
- 与 `T@` 的区别：`T@1` 无引用图、无共享、8 字节、单一所有者；`T@` 24 字节、可多指针共享同一目标。
- 与 `T&` 的区别：仅多了退域/重赋的自动 `drop`+`free`，其余读写、解引用、`p->m()` 全沿用裸指针路径。
- **内建 `com@1`（io 设备句柄）特例**：`com` 的析构是每对象方法指针 `close`（设备 close
  自负全权回收，含结构体本身），故退域/重赋发 `if(c){c->close(c);}`——调 `close` 而非
  `T_drop`，且**不**叠加 `free`（避免双重释放）。于是 `var c: com@1 = file(...)` 后无需手动
  `c->close()`，退域自动收尾，令 io 读写「不必封装收尾函数、直接用 com」。
- **分身/切片句柄 `com[...]`（有界读视图）退域自动 free**：函数内 `var s: com[...]` 退出
  作用域时，若 `s._` 非 nil 自动发 free（等价 `s = nil`），无需手动 `s = nil` 收尾。显式
  `s = nil` 后守卫 `if(s._)` 跳过（幂等）。句柄总在设备（`com@1`）之前释放，故 `com@1` +
  `com[...]` 组合整读整写全程零手动收尾（见 [op.sc](builtins/op.sc) com 机制【B】）。
- **句柄缓冲取走 `s.take()`（零拷贝转移所有权）**：`c >> s` 整读后 `var buf: char& =
  (s.take(): char&)` 把句柄内部数据缓冲摘出交调用方（sc 池缓冲，用 mem.sc `recycle` 释放），
  s 退域只回收元数据、不双释；read-all 缓冲末尾恒 NUL，取走的文本缓冲可直接当 C 字符串。
  糖展开为 `s._->_self->take(s._->_self, s._)`（转发句柄绑定本体 com 的每对象方法指针）。
  与 `com@1` 合用即「整读文件到自有缓冲」零封装、零拷贝、零手动收尾。

> **典型收益**：tensor / nn 模块（`tensor@1` / `linear@1` / `optim@1` …）批量声明后,
> 文末整段 `->drop()` 释放链可直接删除，由退域 RAII 接管。注意 autograd 的 `val&` 节点由
> `nn_tape_clear()` 统一回收，**不可**改成 `@1`（会与 tape 双重释放）。
> io 设备（`file`/`stream`/`tcp`/`ssl` 返回 `com&`）同理可用 `com@1` 免手动 `close`。

### 5.4 瘦指针 `T*`（真瘦自动指针：只统计入边，不带 own 出边）

`T*` 是与胖指针 `T@` **独立的类型**，物理 24 字节 `sc_thin {p, tar, dtor}`：只统计
目标入边 `tar`，**不带/不统计持有者出边 `own`**（比 `sc_afat` 32B 省 8B）。dtor 随句柄持
有，归零时自析构释放，裸 `*` 可类型擦除。适用于「持有方自维护生命周期、无需
own 归属记账」的场景（如容器 item）。

- 与胖指针 `T@`、裸 `@` **可相互赋值转换**（前 16B `p,tar` 同构，拷 p/tar，各按己方记账）。
- 与裸指针 `T&` 的关系**与胖指针相同**（取裸 `.p`，绑定取址传染同 `T@`）。
- 不统计 own，不参与「未清出边」校验；目标入边归零仍触 dtor 自析与回收。
...
> 这类符号无需显式 `inc`、以 `::` 前缀即可使用（见 §9「`::` 映射符」）——本文档与 examples
> 因此不再写 `inc stdio.h`。
...
（全局前奏提供者）。编译器在构建/分析时**从入口单元所在目录起，逐级向上游走父
目录**，在每一级扫描该目录的 `.sc`，取**最近的一个含 `@@` 标注的祖先目录**作为
根——无需任何命令行选项（向上限深 64 级，遇文件系统根即止）。同一目录应只有一个
`@@` 根（多于一个时取排序最前者并告警）。

> **向上发现的意义**：根管辖其所在目录整棵子树。项目常把入口/集成单元放在项目根
> 目录（如 `sagent/sagent.sc` 标 `@@`），把消费单元放在子目录（`sagent/src/*.sc`）。
> 单独分析或 `--test` 某个子目录单元时，编译器会向上找到祖先根并施加注入，因此
> **语法插件**在编辑 `src/` 下的消费单元时，根注入的全局对象不再显示为「未定义」
> （插件在文档同目录落临时文件跑 `scc <tmp> --ast`，走的正是这条向上发现路径）。
...
**跨模块签名的指针形态约束（重要）**：消费单元可在自己的 `@` 导出签名里引用根
注入的类型，但**只能以引用/指针形态 `T&`，不能按值 `T`**。原因是代码层的注入位于
**末位** `#include`：消费单元的导出签名会进入其 ABI 头 `scm_<consumer>.h`，而**其它
单元 `#include` 该 ABI 头的时机早于末位的根接口头 `scm_<root>.h`**——此时根类型尚
无完整定义。

- **指针形态 `T&`**：编译器在消费单元 ABI 头内为根类型补一条**前向声明**
  `typedef struct sc_<T> sc_<T>;`，指针成员无需完整类型即可成立，编译通过。
- **按值形态 `T`**：需要完整类型（`sizeof`/布局），前向声明不足，会在 C 层报
  `unknown type name`。故**语义层提前拦截**并给出可读诊断（含跨模块导入链提示），
  强制改用 `T&`。此约束仅针对**消费单元**（`d->exported && !d->external`）——根
  **自身** `@` 导出签名按值使用自己的类型不受限（其完整定义就在 `scm_<root>.h`）。

```sc
# 根 app.sc：@@ + @def cfg + 按值用 cfg 无妨（定义就在本头）
@fnc app_apply: c: cfg              # ✅ 根自身，按值可

# 消费 net.sc（不 inc 根，经注入用 cfg）
@fnc net_send: c: cfg&, n: i4       # ✅ 指针形态：ABI 头补前向声明
@fnc net_take: c: cfg               # ❌ 按值：语义层报错，请改 cfg&
```
...
[examples/feature30/feature30_mod.sc](examples/feature30/feature30_mod.sc)。指针
形态跨模块签名与按值守卫的最小用例见
[tests/cases/rp_ptr/rp_ptr.sc](tests/cases/rp_ptr/rp_ptr.sc)（正向：`cfg&` 前向
声明生效）与
[tests/cases/rp_byval_bad/rp_byval_bad.sc](tests/cases/rp_byval_bad/rp_byval_bad.sc)
（负向：按值触发语义拦截）。
...
- 按扩展名分流：`.c`/`.cpp`/`.cc`/`.cxx` 现场编译为对象文件后链接；
  `.o`/`.a`/`.so`/`.dylib` 直接参与链接；`.sc` 内联（源码拼接，见下「加入 sc 子单元」）；
  其它类型报错。
...
### 加入 sc 子单元（`add x.sc`：源码内联）

`add` 除了并入 `.c`/库，还支持并入另一个 `.sc` 文件。**其本质是两个 `.sc` 文件的
拼接（源码内联）**：被 `add` 的 `.sc` 作为**前置片段**并入本单元，因此其**默认
（未导出、非 `@`，即 static）函数与类型对本单元直接可见**。

用途：把一个单元里大段的**私有辅助函数/类型**抽出、集中到一个独立 `.sc` 文件，
主单元只需一行 `add` 即可复用，而无需把这些辅助逐个 `@` 导出成跨模块接口。

```sc
# helper.sc —— 独立子单元：集中放私有辅助
inc mem.sc                       # 子单元可有自己的 inc/add

def Vec2: { x: i4, y: i4 }       # 私有类型（非 @）

fnc h_add: i4, a: i4, b: i4      # 私有函数（非 @）
    return a + b
```

```sc
# app.sc —— 容器单元：add 后直接调用私有辅助
add helper.sc

fnc main: i4
    printf("%d\n", h_add(20, 22))    # 42：helper.sc 的私有函数在此可见
    return 0
```

内联 vs `inc` 的区别：

| 维度 | `add x.sc`（内联） | `inc x.sc`（模块依赖） |
| --- | --- | --- |
| 机制 | 源码拼接进本单元，前置 | 独立编译成单元 + 链接 |
| 可见性 | **全部**成员（含私有非 `@`）可见 | 仅对方 `@` 导出成员可见 |
| C 产物 | 并入本单元 `.c`，不单独出 `.c` | 各出各的 `.c`，经接口头连接 |
| 定位 | 同单元内的私有辅助复用 | 跨模块公共接口依赖 |

规则与约束：

- **前置拼接**：被 `add` 的内容排在本单元之前，其顶层声明按内联顺序进入本单元。
- **保留 `@` 状态**：被内联的声明保持各自的 `@`/非 `@`；私有(非 `@`)辅助因此对
  容器可见，而 `@` 导出成员仍作为本单元的导出对外。
- **自完备子单元**：被 `add` 的 `.sc` 可有自己的 `inc`/`add`；被内联时其
  `inc`/`add` 一并并入容器。**其相对路径以「子单元自身所在目录」为基准**解析
  （而非容器目录），故 `helper.sc` 里写 `inc sub/x.sc`、`add glue.c` 均相对
  `helper.sc` 的目录。
- **单一 add 约定**：一个 `.sc` 子单元应只被**一个**文件 `add`（人工约定，避免同
  一份实现被并入多处而在链接期重复定义）。链内的 `add` 环会被检测并报「add 循环
  依赖」。
- **级联/传递（A → B → C）**：`add` 可多层级联——被 `add` 的子单元自身可再 `add`
  别的 `.sc`，编译器**递归内联**整条链，最外层容器因此能直接使用最底层子单元的
  私有成员。归属逐层精确：某成员的来源始终记为**它真正所在的文件**（如经 B 并入 A
  的 C 成员仍归属 `c.sc`，而非中转的 `b.sc`），故 `#line` 调试与 IDE 跳转都落到真正
  源文件。环检测贯穿整条链（`A add B`、`B add C`、`C add A` 会报「add 循环依赖」）。
- **调试映射**：内联函数体在生成的 C 里带 `#line` 指回原始子单元文件与行号，故
  编译错误与调试器定位落在 `helper.sc` 而非拼接后的中间产物。
- **源码往返**：`--emit-sc` 原样保留 `add helper.sc` 指令（不展开内联），保证
  round-trip；`--ast` 则展示内联后的成员，并在节点上标注来源子单元文件，供
  IDE（AST 视图）跳转到 `helper.sc`。
...
- C 宏常量（`::INT_MAX`、自定义 `::OS_NAME` 等）以 `::` 前缀作为标识符使用，原样透传到生成的 C 代码。
- C 函数式宏（`::SQUARE(x)`、`::va_start` 等）以 `::` 前缀按函数调用语法使用。
...
**`::name` —— 调用/取用 C 域的函数、宏、符号**

sc 拥有独立命名域（sc 名统一以 `sc_` 前缀落到 C），因此**表达式位置引用任何 C 侧名字都必须显式加 `::` 前缀**——
无需事先声明，但前缀不可省：`::printf` 调用 C 函数、`::SQUARE(x)` 展开函数式宏、`::INT_MAX` 读取 C 宏常量
（前提是该名字所在头已 `inc` 进来，或属编译器默认带入的标准头）：
...
> 裸写 C 名（如 `printf` / `INT_MAX`）不会再「落到 C」，而是编译期报错
> 「未声明的符号（sc 命名域）：若引用 C 域符号请写 '::xxx'」——这是命名域隔离的刻意设计
> （早期版本曾对未解析头的名字放宽未定义检查、裸名直接落到 C，即 `lenientCalls`，现已移除）。
...
> 如 `sys`/`io`/`adt`）是一个特例：其 `.h` 由**手工编写并与 `.sc` 同步维护**，承担「导出
...
> `inc sys.sc` 后写 `mix ARGS_B(abc, ...)`，该宏内部定义出全局 `ARGS_abc` 与 `ARGS_DEF_abc`，
...
**builtins 是透明特例。** `adt`/`io`/`mem`/`sys`/`m`/`async`/`os` 等内置模块的全部函数均为
...
**为什么要它（核心目的）。** 用来定义**具有唯一身份的逻辑对象**——身份不可复制，只能被引用。
没有 `Name` 值实例，就**没有可按值拷贝的对象**，从根上**杜绝值赋值**：天然规避「拷贝
一份内部持有指针/自动指针的结构体」带来的双重释放/记账错乱（与 §25.1 的设计思想同源——
让「有唯一身份的逻辑对象」与「无身份的数据块」各走各路，并把约束前移到语言规则）。共享只能经 `Name&`
...
| `p->drop()` | `sc_ptr_drop_slot(&p, Name_drop)` | nil 安全；先置 `p=nil`，再执行 `Name_drop`（若有）+ `free(p)` |
...
- 对可寻址堆指针，`p->drop()` 由编译器转为一次安全析构：检查 `p != nil`、先清空 `p`、
  再调用用户 `drop` 和 `free`。因此 `p->drop(); p->drop()` 不会二次释放。若接收者是不可
  寻址的临时表达式，只能执行 nil 安全析构，不能清空临时来源。
...
- `init`（无参）：**声明即构造**。函数内 `var x: T` 且无显式初值、
  ...
  **静态全局对象**（文件作用域 `var g: T`，非指针/数组/`tls`）若 `T` 含无参
  ...
  + 构造体；详见下文「静态全局对象生命周期」）。
  **含自动指针 `T@` 成员的类型**，其 `init` 可经 `this` 给胖成员绑「新边」
  （`this->child = T()`，首次构造子结构）：编译期给此类 `init` 加一个隐藏形参
  `_self_own`（容器自身的 `out` 指针），由建对象处传入——ref 容器传 `&owner.out`
  → 子对象带头记账纳入引用图、随 owner 归零沿 `drop` 递归回收；无头容器（栈/裸
  堆/静态）传 `SC_OWN_RAW` → 子对象**退化为裸创建**（不追踪，手动生命周期）。
  析构 `drop` 只清不建（详见 §自动指针 7.4）。
- `drop`：析构。**栈值对象退域时自动调用**（RAII）：函数内 `var x: T`
  ...
  裸指针对象（`T&`）的 `drop` 仍需手动调用；堆专属裸指针的显式 `drop` 经过 nil 守卫，
  可寻址指针槽会在析构前置为 `nil`，因此重复调用安全。**自动指针 `T@`** 的 `drop` 由
  ARC 在堆对象**入边归零（`in→0`）时自动触发**——先析构（由 `drop` 经 `this->m = nil`
  清理子成员出边，可逐层递归），再判 `out` 决定回收/报“未清理”（详见 §自动指针）。
  值对象的显式 `drop` 仍依赖类型实现的状态重置，不能把所有用户 `drop` 视为幂等。
  **静态全局对象**的 `drop` 由编译器在 `main` 尾声自动调用（详见下文）。
...
sc 提供两种「函数体内的可复用逻辑块」，机制截然不同：**伪闭包**（匿名
函数字面量，提升为独立函数）与**真内联**（`inl` 命名块，调用点原地展开）。
...
`inl` 在**函数体内**定义一个命名代码块，调用点被**原地展开**（等价于
把块体文本粘贴到调用处），与外层**共享同一栈帧**——本质就是 C 的一对
`{ }` 加上「按名复用」。它对应 C 的 `inline`，但语义是「文本内联」而
非「独立函数」：定义处不生成任何函数，调用点也不产生 call。

```sc
fnc classify: i4, n: i4
    var acc: i4 = 0
    inl step:                 # void inl：无返回类型，当语句用
        if n < 0              # 直接读写外层局部 acc / n（共享栈帧）
            acc = -1
            return            # 裸 return = 早退本块（不返回外层函数）
        acc = n * 2
    step()                    # 调用点原地展开
    return acc

fnc doubler: i4, x: i4
    inl dbl: i4, v: i4        # 值 inl：签名声明返回类型 i4（同 fnc 写法）
        if v == 0
            return 100        # return v → 把 v 赋给左值并早退本块
        return v * 2
    var a: i4 = 0
    a = dbl(5)                # 值上下文：赋值右侧
    let b: i4 = dbl(0)        # 值上下文：var/let 初始化
    return a + dbl(b) - 100   # 也可嵌入更大表达式/作实参（自动临时提升）
```

语法：`inl 名: [返回类型,] 形参... `（签名与 `fnc` 一致，冒号不可省；
无参无返回也写 `名:`），换行后缩进块体。
...
| 形态 | 定义 | 用法 | `return` 语义 |
|------|------|------|---------------|
| **void inl** | 无返回类型（`inl name:`） | 只能当**独立语句** `name(args)` | 裸 `return`（无值）→ 早退本块 |
| **值 inl** | 有返回类型（`inl name: T, …`） | 当**值**用（也可当语句丢弃返回值） | `return v` → 目标 `= v` 后早退本块 |

值 inl 可出现在**任何无条件求值**的位置：赋值右侧 `lhs = name(args)`、
初始化 `var|let x = name(args)`、`return name(args)`、更大表达式内
（`a + name(args)`）、普通函数或 inl 的实参（`f(name(args))`、
`name(name(args))`）、`print`/`assert` 实参，也可作**独立语句**
`name(args)` 丢弃返回值。整体作赋值/初始化右值时**直接赋左值**（快路径，
无临时）；嵌套在更大表达式里时，先展开到自动命名临时 `_inlN` 再参与外层构造。

**位置限制**：值 inl **不能**出现在会重复求值或条件求值的位置——循环/`if`
条件、`&&`/`||` 右侧、三元 `?:` 分支——否则报错（会改变求值次数/语义）；
如需在这些位置使用，请先用中间变量保存其值。

要点与约束：

- **共享外层栈帧**：块体可直接读写定义点之前可见的局部变量（与伪闭包相反）。
- **形参绑定一次**：实参在展开处求值一次，绑定到同名块级临时；块体多次
  引用形参不会重复求值（无宏式重复副作用）。字面值实参走**快路径**直接
  文本代入（不生成临时）。
- **不可递归**：内联块直接或间接调用自身会报错（文本内联无法自我展开）。
- **先定义后使用**：调用必须在定义之后（同一函数体内按出现顺序）。
- **返回类型强校验**：值 inl 的 `return v` 与左值类型都按声明返回类型
  做语义检查；值 inl 内**裸 `return` 报错**，void inl 内**带值 `return` 报错**。
- 转 C 时块体展开为 `{ 形参临时=实参; …块体; 尾标签:; }`，`return`
  编译为 `goto 尾标签`（值 inl 先 `目标 = v;` 再 `goto`）。嵌套作子表达式
  时先展开到自动临时 `_inlN`（块内 `return v` → `_inlN = v; goto`），
  原位以 `_inlN` 引用。`--emit-c` 可见展开结果，`--emit-sc` 原样保留 `inl` 定义。

#### 伪闭包 vs 真内联

| | 伪闭包 `fnc: …` | 真内联 `inl name: …` |
|---|---|---|
| 出现位置 | 表达式（赋给函数指针/传参） | 语句（函数体内定义命名块） |
| 转 C | 提升为顶层 `static` 函数 | 调用点原地展开，无独立函数 |
| 外层局部 | **不可**捕获（仅参数/全局） | **共享栈帧**，可读写 |
| 调用开销 | 一次函数指针调用 | 零（内联展开） |
| 可否递归 | 可（是真函数） | 否 |
| 返回值 | 普通函数返回 | void：早退；值：赋左值后早退 |
...
调用，返回 rpc 结果）；阻塞投递给队列并等回复用 `sync<q> work(args)`；
另有 `async`（登记进事件循环，返回 future）、`run`（独立线程/线程池）、
队列 `<<`（不带回复投递）等形态。裸 `rpc()` 调用会编译报错。
...
完整语义（`future` 类型、事件循环、`done` 兑现）见 §15.4。
...
- 三元条件：`cond ? a : b`
- 括号分组：`(expr)`
- 异步前缀：`async rpc(...)`（登记到事件循环，返回 `future&`）、`await expr`（挂起待 future 就绪，见 §15.4）
...
- `break`
- `continue`
- `final`（作用域退出钩子，见 §15.7）
- `run`（以 rpc 调用创建线程，见 §15.2）
- `done`（标记 future 就绪并唤醒等待者，见 §15.4）
- 变量/常量声明
- 表达式语句
...
fnc main: i4
    ...
    run<p> work(&c, 10000)     # 入池：任务排队，由工作线程执行
    p->join()                  # 屏障：等全部已提交任务完成
    p->drop()                  # 析构：等任务完成后停池回收
    return 0
```

- **目标 `<target>`（可选，首项）**：`<p>`（`pool&` 接口协议，值则自动取址）
  → 任务入池；省略 `<>` → 独立线程（detach 或经 `&t` 出参 joinable）。
  目标在尖括号内（与 `print<chn>` 对齐），其后可跟 `key:val` 选项。
- **出参 `&t`（可选，尾随逗号参数）**：`&t`（t 为 `thread&`）→ joinable
  独立线程，必须 `join()` 等待并回收；省略 → 线程内部 detach，结束后自释放。
  仅独立线程可带 `&t`（入池目标不接受）。
...
- 转 C：装填 rpc 参数结构体后按 `<target>` 类型改发——独立线程
...
`run` 的选项块除首项可放目标（见上）外，还可带 `key:val` 选项设置线程属性，
透传给 C 由 m 模块的 `thread_run` 具体实现（值为**任意表达式**，运行时求值，不做
编译期范围校验）。当前支持两个键（均可选、顺序无关）：
...
### 15.3 并行协同（sync / async / done）

与 §15.2 `run` 的「裸线程/线程池创建」互补：本节是基于**消息队列 `queue`**
的协同层——把 rpc 调用打包成消息在线程间传递，提供即发即忘（`<<` 投递）、
阻塞带回复（`sync`）、非阻塞带回复（`async` → `promise`）三种跨线程调用形态，
以及优先级/延迟投递选项。均需 `inc mt.sc`。`queue`/`pool`/`promise` 同属 op 层接口
协议（vtable，默认导入），mt 模块按默认策略提供具名构造（`default_queue` / `default_pool`），
故全部经协议指针派发、语言内核零 emit mt 符号。本节用到的 `async`/`done` 是「队列
协同」语境；其单线程协作式异步（事件循环 + `future`）语义见 §15.4。
...
- **宿主三态**（构造时绑定 `host: pool&`）：`nil` 未绑/延迟、`main` 当前/主线程
  （手动 `pull`）、`&pool` 线程池消费（`<<` 投递自动转交 `pool->run` 并发消费，
  用 `p->join()` 作屏障，无需 `pull`）。`main` 是 op 提供的 `pool&` 哨兵常量（值 -1）。
...
- 转 C：`<<` 经协议指针派发 `q->post(q, work_rpc, &参数, sizeof)`，语言内核零 emit
  ...
  宿主为真实池时 `q->post` 内部转发 `host->run`（仍为协议指针派发）。
- 示例：`nil`/`main` 手动 `pull` 见 `examples/feature38.sc`；`&pool` 自动并发消费
  见 `examples/feature39.sc`。

#### 队列阻塞调用 `sync<q> work(args)`（带回复）

`sync` 除「无目标 = 当前线程直接执行」外，还可在选项块首项带一个队列目标：把 rpc 调用
**阻塞投递**给 `q`，阻塞至某消费者（另一线程 `pull` / 线程池工作线程）执行
完成，取回 rpc 的返回值：
...
var q: queue& = default_queue(p)      # 线程池宿主
var r: i4 = sync<q> compute(3, 4)     # 阻塞 → 池工作线程算 7 → 取回
q->drop(); p->drop()
```

- **目标在 `<>` 内**（与 `print<chn>` 对齐）：`sync<q> work(args)` 的 `q` 是队列目标。
  尾随逗号参数仅用于状态出参 `, &st`（见下）；故在实参列表内用 sync 须加括号：`f((sync<q> w()), x)`。
- **消费者前提**：`q` 一般由**别的**线程/池消费（`default_queue(pool)` 或另起一
  线程跑 `q->pull`）。**循环死锁替代（P5d）**：消费者即「谁 `pull` 它」（运行时在 `pull`
  时把队列惰性绑给当前线程）。当线程向「自己消费的队列」或「沿消费链回到自己」（A 消费
  QA 并 `sync` 到 QB、B 消费 QB 并 `sync` 回 QA 形成互锁环）的队列 `sync` 时，直接投递+阻塞
  会死锁——运行时改为**在当前线程直接执行该 rpc（替代）**打破循环，立即取回结果。无限等
  与有限超时两路同效；线程池宿主队列无 `pull` 消费者（恒空）故替代天然禁用、照常转交池。
- **有限超时 + 状态出参**：`<q, prio:N, delay:ms>` 选项见下文「队列消费的优先级
  与延迟」小节；另有 `timeout:ms` 把「无限等」升级为「有限超时」（P5c）：
  `sync<q, timeout:100> work(args)` 至多等 100ms，超时则放弃等待。可附加状态
  出参 `, &st`（仿 `run ..., &t`，`st: i4` 左值）区分超时 vs 成功：
  ```sc
  var st: i4
  var r: i4 = sync<q, timeout:100> compute(3, 4), &st   # 状态码 0=成功 / 1=超时 / -1=队列关闭
  ```
  表达式仍求值为返回槽（超时时为 0，未被回填）。状态出参**须** `&var` 形式且**须**
  同时带 `<timeout:ms>`（否则编译错：无限等下状态恒为成功，无意义）。
  实现：无限等路径在调用方栈上放描述符（零分配）；有限超时路径改用堆盒子 +
  引用计数——执行方只在盒子堆缓冲上跑 rpc，绝不碰调用方栈，故调用方超时 abort 不致 UAF。
  循环死锁替代（P5d，见上「消费者前提」）发生在 sync 实现内部：命中互锁环即本地执行、
  不投递不阻塞——编译器仍零 emit mt 符号。
- 转 C：语句表达式求值 `({ struct W _rp = {0}; 填实参; q->sync(q,(void(*)(void*))W_rpc,&_rp,sizeof(_rp),prio,delay,timeout); _rp._; })`
  （带状态出参时首部作 `st = q->sync(...)`）——结果由消费者回填返回槽 `_rp._`，语言内核零 emit mt 符号。实现上 `q->sync`
  把 (fn, 参数缓冲指针, 应答描述符) 打成 trampoline 经 `post` 投递，消费者在调用方的
  参数缓冲（或超时路径的堆盒子）上实跑 rpc 后置位并唤醒调用方。
- 示例：线程池宿主 + 专用消费线程两种消费路径见 `examples/feature40.sc`；有限超时（成功/超时两路）见 `examples/feature43.sc`；循环死锁替代（A↔B 互锁环 + 自替代）见 `examples/feature44.sc`。

#### 队列非阻塞调用 `async<q> work(args)`（返 promise&）

`async` 除「无目标 = 登记进事件循环（libuv 协作式，见 §15.4）」外，还可在选项块首项带一个队列
...
var f2: promise& = async<q> compute(100, 23)  # 可连发多个，再逐个取
...
- **`promise`（mt-future）**：线程世界的阻塞型未来，与 `future`（§15.4 libuv 协作式、绑
  事件循环）不同。方法：`p->ready(): bool`（非阻塞轮询）、`p->wait(): T`（阻塞取结果，
  类型擦除 `void*`，调用点 `: T` 还原，与 `future.get()` 同语义）、`p->drop()`（释放）。
  `promise&` 是协议指针，方法用箭头。
- **目标在 `<>` 内**（与 `print<chn>` 对齐）：`async<q> work(args)` 的 `q` 是队列目标。实参列表内用须加括号。
...
#### 优先级与延迟选项 `sync<q, ...>` / `async<q, ...>`

`sync` / `async` 驱动 rpc 入队时，选项块首项为队列目标，其后可跟 `key:val` 选项（仿 `run<...>`），
取值为**任意表达式**（运行时求值，不做编译期范围校验），多个以逗号分隔：

```sc
async<q, prio:5> work(args)          # 高优先级：先于低优先级被消费
async<q, delay:50> work(args)        # 延迟 50ms：到期后才可被 pull
sync<q, prio:3, delay:10> work()     # 两者可同时给
```

- `prio`——就绪链按优先级排序，`pull` 总取最高优先级；同优先级稳定 FIFO。缺省 0。
- `delay`——入「延迟链」按到期时刻升序；`pull` 阻塞至最早到期项成熟再消费。缺省 0。
- `timeout`（仅 `sync`）——把阻塞等回复从「无限等」限为「最多等 N 毫秒」（P5c）；
  超时则放弃等待。可配状态出参 `, &st` 区分超时/成功（见上「队列阻塞调用」小节）。
  `async` 暂不支持 `timeout`（给了报编译错）。
...
- **`<<` 不带选项**（语法上与移位/比较冲突）：要「延迟/带优先级的即发即忘」，
  用 `async<q, ...>` 驱动并丢弃返回的 `promise` 即可。
- 转 C：选项经协议指针透传 `q->post/sync/async` 的 `prio`/`delay_ms`（`sync` 额外 `timeout_ms`）形参，语言内核零 emit mt 符号。
- 示例：优先级与延迟排序的确定性观测见 `examples/feature42.sc`；`sync<timeout:ms>` 有限超时见 `examples/feature43.sc`。

### 15.4 异步协程（async / await / done）
...
`sync<q> work(args)` 把 rpc 投递给队列消费者执行并**阻塞等回复**——默认由 rpc 体
...
fnc main: i4
    ...
    var st: thread& = nil
    run server(sq), &st
    var r: i4 = sync<sq> serve(7)   # 阻塞至延迟应答兑现：r=70（而非 rpc 体的 return 0）
```
...
sc 在语言层内置了一套**设备通讯**机制框架，具体包括：
- 内置类型 `com`（通讯端点）
- 两个流操作符 `<<`（发）/ `>>`（收），用来实现把"把一组值发到设备 / 从设备收一组值"。
- 内置 O(1) 性能的多路复用异步 I/O 就绪通知能力，并和语言层的 async 异步机制整合。具体为：
  - Linux: epoll
  - macOS·BSD: kqueue
  - Windows: IOCP
  - 其它 POSIX 兜底: poll

sc 仅提供了通讯机制框架，具体通讯设备则由 buildins 模块或用户自行实现（如 `inc io.sc` 的 `file/stream/tcp` 文件设备）。
...
- **异步形态（`rpc` 内）**：`rpc` 体内出现 `<<` / `>>` 即把该 rpc 标记为异步
  （等同含 `await`，见 §15.4），编译为状态机。每个收发点是一个 `await` 切点：
...
| `future` 异步结果句柄（init / ready / get / ctx） | §15.4 |
...
- **支持跨模块单态化**：泛型 `def` 模板对依赖单元可见（随 `inc` 透传），各单元可独立 `mix`
  同一泛型；编译器把所有单元的实例**聚合进工程级 `generic.h`**（按类型名去重、跨单元一致），
  自包含实例（字段仅基本类型/指针）写出完整定义、其余给前向 `typedef`。故一个模块可导出形参为
  泛型实例类型（如 `@fnc f: v: Vec_int&`）的函数，消费单元跨模块直接调用（见 §18.4 末示例）；
- 不写 `<...>` 的 `def` 宏仍是上节的文本宏（保持 `#define` 行为，向后兼容）。

> 完整可运行示例见 [examples/feature33.sc](examples/feature33.sc)（单模块）与
> [examples/feature38/feature38.sc](examples/feature38/feature38.sc)（**跨模块**：附属模块
> 定义/实例化模板并导出实例签名函数，入口跨模块调用并共享同一 `Vec_int`）。
> `--emit-c` 可见生成的是具体 `struct Vec_int { … }` 与具体函数，而非 `#define`；
> 跨模块时实例类型集中落在同级 `generic.h`。

> **注意（跨模块按值 vs 按指针）**：`generic.h` 最先被 include（早于用户模块头），故只有
> **自包含实例**——字段全是基本类型/指针（如 `def Vec_int: { data[8]: i4, len: i4 }`）——
> 才把完整定义写进 `generic.h`，跨模块**按值或按指针**都能传：
> ```
> @fnc sum: i4, v: Vec_int      # ✅ 自包含：按值跨模块 OK
> @fnc push: v: Vec_int&, x: i4 # ✅ 按指针更 OK
> ```
> 而**非自包含实例**——按值内嵌了用户结构体（如 `Vec_pt` 内含 `data[8]: Point`）——其完整
> 定义依赖 `Point` 的定义（而 `Point` 在用户模块头里、晚于 `generic.h`），故 `generic.h`
> 只给它前向 `typedef`，跨模块**只能按指针传**：
> ```
> @fnc each: v: Vec_pt&         # ✅ 按指针：前向 typedef 够用
> @fnc copy: Vec_pt, v: Vec_pt  # ✗ 按值：消费方看不到完整定义，编译失败
> ```
> 这并非能力硬伤（把 `Point` 也聚合进 `generic.h` 即可解，但不值当）：内嵌用户聚合的大对象
> 本就该传指针。需要跨模块按值传的容器，让其元素是基本类型（自包含）即可。
...
即同一消息 → 天然多态）；维度恒返回三态 `tril`。完整可运行示例见 `examples/feature37.sc`。

### 19.1 tril 三态类型

新增基础类型 `tril`（与 `bool` 同级），C 落为 `int8_t`。三个内置字面量（与 `true`/`false`/`nil` 同类）：
...
cls Cat: {
    ...
    init: fnc
        this->age = 3
    dim SPEAK: tril, buf: char&, cap: i4     # 出参 buf 回填叫声
        snprintf(buf, cap, "喵")
        return positive
}
```

**五个保留维度**（选择子 0..4，用户 dim 从 5 起）：
- `CLS_ID`（出参 `int32_t* id`，写 `SC_CLS_<T>`，不可覆盖）；
- `OBJ_KEY`（出参 `void** key`；类含 `obj_key` 字段时默认取字段值，否则取对象基址；可覆盖）；
- `OBJ_NAME`（出参 `char* buf, int32_t cap`；类含 `obj_name` 字段时默认 `%s` 取字段值，
  否则 `snprintf "<T>@%p"`；可覆盖）；
- `RLT_KEY`（入参 `object other`，与另一同类对象比对 key，默认经 `OBJ_KEY` 取值后按地址比大小，
  返回三态 `negative/unknown/positive`；可覆盖）；
- `RLT_NAME`（入参 `object other`，与另一同类对象比对 name，默认经 `OBJ_NAME` 取值后 `strcmp`，
  返回三态；可覆盖）。

对象未实现的维度 → `default` 返回 `unknown`。**同名维度是同一全局消息，语义检查强制其参数签名一致**
（参数个数与各参数类型逐一相同），否则报错。

**维度调用**：静态接收者 `o.DimName(args)`（`T`/`T&`/`T@`）→ `T_hyper_impl(&o._class, SC_DIM_DimName, args)`；
动态接收者 `ob.DimName(args)`（`object`）→ `(*ob)(ob, SC_DIM_DimName, args)`。

### 19.3 object 类型擦除引用与 instanceOf

`object`：指向任意类对象 `_class` 槽的指针（C 即 `typedef sc_hyper *object`），既是身份（地址唯一）
又是分派入口（解引用即得分派器）。特殊强转 `(o: object)` ≡ `&o._class`。

`instanceOf(o: object, TypeName) -> bool`：O(1) 身份判定（比较 `*o == TypeName_hyper_impl`）。

```sc
var c: Cat
var pets[1]: object
pets[0] = (c: object)              # 类型擦除
var sound[16]: char
pets[0].SPEAK(&sound[0], 16)       # 动态多态分派
if instanceOf(pets[0], Cat)
    printf("是 Cat\n")
```

### 19.4 跨模块一致的类 id 与维度选择子

全局类 id（`SC_CLS_<T>`）与维度选择子（`SC_DIM_<Name>`）**跨编译单元一致**：工程/多文件编译
（`scc 入口.sc`、`scc build`）时，编译器取所有单元的类名/维度名并集，生成一份共享头 `class.h`
（含 `tril`/`sc_hyper`/`object` 类型定义与 `SC_CLS_`/`SC_DIM_` 枚举），各单元 `.c` 在文件头
`#include` 之；导出类机制类型的模块头亦 `#include "class.h"`，使导出的 `cls` 结构 `_class`
字段与 `object` 形参可见。故同名类/维度在不同单元取**同一编号**，跨模块动态分派与 `instanceOf`
均正确（仿 `future<ID>` 的 `type.h` 聚合机制）。单文件 `--emit-c`（标准输出）走自包含内联模式。

`cls` 实例的 `_class` 分派器指针在所有构造点自动安装：函数内局部实例（声明处，早于 `init`）、
堆构造 `T()`、自动指针 `T@`，以及**全局（非函数内）实例**（模块初始化序言，早于全局 `init`，
故全局对象同样可经 `object` 擦除做动态多态分派与 `instanceOf`）。

## 20. 模块单例 `mod`（声明即创建的单例对象）

`mod` 声明一个**模块**——其本质是一个**单例对象**。`mod N:` 等价于「定义单例类型 `N_m` + 创建
唯一实例 `N`」：**声明即创建**（复用全局对象生命周期，自动构造 / 析构）。它把「一组状态字段 +
操作这组状态的成员函数」打包为一个有名字的全局单件，是组织模块级状态与行为的首选形态
（配置中心、日志器、注册表、设备管理器等）。

```sc
@mod config:                        # @mod → 类型 config_m 与实例 config 均导出（extern）
    lvl: i4                         # 模块状态字段

    fnc init:                       # 零参 init → 进 main 前自动构造
        this->lvl = 1
        return

    @fnc set_level: n: i4           # @fnc → 导出成员函数，跨模块可调
        this->lvl = this->clamp(n)  # this-> 访问字段 / 调成员函数
        return

    @fnc level: i4
        return this->lvl

    fnc clamp: i4, n: i4            # 无 @ → 私有成员函数（static），仅本单元可见
        if n < 0
            return 0
        if n > 5
            return 5
...
```sc
config.set_level(3)                 # 跨模块调用 → config_m_set_level(&config, 3)
var lv: i4 = config.level()
```
...
### 20.2 构造 init / 析构 drop

- 单例类型 `N_m` 若有**零参** `init` 成员函数 → 进入 `main` 之前**自动构造**（复用静态全局对象
  生命周期，见 §11.3）；若有 `drop` → `main` 尾声**自动析构**。
- `init` / `drop` 是单例的内部生命周期，**禁止 `@` 导出**——语义上不可跨模块手动调用（由编译器
  自动驱动）。写 `@fnc init` / `@fnc drop` 即编译期报错。

### 20.3 导出 `@mod` vs 私有 `mod`

| 形态 | 类型 `N_m` / 实例 `N` | 成员函数导出 | 可见性 |
|------|----------------------|--------------|--------|
| `@mod N:` | 导出（`extern`） | 成员可逐个 `@fnc` 导出 | 跨模块（其它单元可 `inst.method()`） |
| `mod N:`（无 `@`） | 私有（`static`） | **不得**导出成员函数 | 仅本单元 |

- 导出由 `@` 控制：`@mod N` 把单例**类型与实例**一并导出（其它单元经 `inc` / `@inc` 见到 `N` 与其
  导出成员，`inst.method()` 展开为 `N_m_method(&inst, …)`）；`mod N`（无 `@`）为私有模块，且其成员
  函数不得再 `@` 导出（私有单例的成员对外无意义）。
- 成员级 `@`：`@mod` 内逐个成员函数以 `@fnc` 决定是否导出；未导出者为模块私有（`static`），只能被
  同单元 / 本模块其它成员调用。

### 20.4 兄弟模块互相可见（`@inc`）

与 `@@` 根模块配合：根（集成单元）以 `@inc`（导出式 `inc`）引入的各 `.sc` 模块互为**兄弟**，其 `@`
导出默认**彼此可见**——无需兄弟之间逐一回引。普通 `inc` 仅为根的私有依赖，不参与互见。

```sc
# app.sc（@@ 根 / 集成单元）
@@
@inc config.sc                      # 兄弟 1（@inc → 参与兄弟互见）
@inc logger.sc                      # 兄弟 2：内部可直接引用 config 的导出
```

> **术语区分**：本章的 `mod`（**单例对象**）与「编译单元 / `.sc` 文件级模块」（其生命周期函数
> `sc_mod_<文件>_init/drop`，见 §7 / §11.3）是两个不同概念——前者是语言里的单件对象构造，后者
> 是每个 `.sc` 单元的模块级全局构造 / 析构入口。完整示例见
> [examples/feature46/feature46.sc](examples/feature46/feature46.sc)。

## 21. 分布式 token 体系 `tok`（观念量 / 依赖图 / form）

`tok` 声明一个**分布式 token**（观念量 / notion）：以字符串 id 唯一标识的共享量，值类型擦除为
`@`（裸自动指针，读出时由调用点 `(e: T@)` 还原；裸指针 / 整数可经 `(e: @)` 装箱）。核心理念是
「**设值即广播、依赖即重算**」——`set` 不只是写值，还会触发挂在该 token 上的依赖关系（`dep`）级联
重算（**唯新值 ≠ 原值才传播**：值不变则静默丢弃；combine 体内 `return tok_modified()` 可在合成
不变时强制刷新一次）。一组 token 经 `dep…map` 连成**显式依赖图**（工作流 / 流水线 / 神经网络），编译器在**编译期**
对这张静态图跑图算法、把结构度量**烘焙**为常量，运行时只是 O(1) 查表。协议（`@def token`）声明在
`builtins/op.sc`，C ABI 与运行时在 `builtins/tok/`，经 op→tok 隐式依赖始终随工程链接，无需 `inc`。
完整机制详见 [builtins/tok.md](builtins/tok.md)。

### 21.1 `tok` —— 声明 token 句柄

`tok t: "id"` 声明一个 token 句柄（`token&`）。紧随的**缩进块**（无引导冒号）即 **combine 体**：

- **有体 = form 形成主**：输入先经 combine 回调合成再落值（峰值保持 / 去抖 / 累积等）。combine 体
  唯一上下文形参 `this: __sctok_in&`，成员皆 `@`：`this->base`（当前值）/ `this->input`（本次
  输入）/ `this->sender`（发送者，恒空 `@`）/ `this->tag`（`set` 随附标签 `i4`），返回 `@`（新值）。
- **无体 = enforce 纯从**：`set` 直接赋值，无合成。

> **就绪唯凭 `form`**：无论有无 combine，token 皆须经 `form` 语句**激活**（共享量的「主启动点」）后方
> 就绪；bind 仅取共享壳、不就绪。未 `form` 的 token 其 `set` 仅入挂起队列、暂不落值/传播，待 `form` 后回放。

```sc
tok level: "sensor.level"           # form 形成主：combine 取较大者 = 峰值保持
...
tok alert: "sensor.alert"           # enforce 纯从（无 combine，set 直接赋值）
```

### 21.2 `form` —— 灌初值 / 升格形成主

`form t, v` 是语言内置语句（与 `run` / `done` 同级）：给 token 灌初值（`@`）、升格为**就绪主**（共享量的
启动点），回放 form 前挂起的 `set`，触发就绪依赖。对**任何**自有 token 均适用（不限 combine 形成主）：
未 `form` 的 token 不就绪，其 `set` 不传播；enforce（无 combine）`form` 即直赋型主。

```sc
form level, (0: @)                       # 灌初值 0 并升格为 form 主
form gray,  (0: @), &gray_n              # 可选第三参 ctx：绑定节点私有上下文（侧车）→ t.ctx() 取回
form gray,  (0: @), &gray_n, node_exec   # 可选第四参 exec：挂节点处理钩子（拉取 back / 推送 set 共用）
```

可选第三参 `ctx`（`&n` 侧车）+ 第四参 `exec`（`token_exec_fn` 钩子）把节点私有状态与处理函数都经
`form` 绑定到 token——`form` 是节点配置的**唯一入口**，配合 `t.ctx()` 实现「dep 管路由，node 管函数」
（见 `builtins/tok.md` §1.3.5 与 `templates/` 拉取/推送两模板）。另见 `t.pulse(v, tag)`：绕过 `set` 的同值
抑制、强制传播，用于拉取流水线/迭代（`builtins/tok.md` §1.4）。

### 21.3 `dep` + `all` / `any` —— 依赖关系（门逻辑）

`dep all/any: …` 声明 token 间依赖，`follow` 回调在依赖项变更 / 就绪时触发：

- `all` = **与门**（全部依赖项各变更过一次才触发）；`any` = **或门**（任一变更即触发）。二者为
  上下文标识（非保留字）。
- 依赖项写作 `<局部名>:"<id>"`，可行内逗号分隔，或块形态（每行一项，`-` 单独成行分隔依赖项与
  follow 体）。各局部名 `a:"id"` 由编译器注入糖 `var a: token& = this->toks[i]`。
- follow 体唯一上下文形参 `this: __scdep_in&`：`this->toks`（依赖项数组 `token&&`）/ `this->count`
  （个数 `i4`）/ `this->active`（触发动作码 `i4`，负=门事件、≥0 为或门变更项下标）。返回 `bool` =
  下次门逻辑（`false`=或门 / `true`=与门，可动态切换）。

```sc
dep any: l:"sensor.level"           # 或门：level 变更即评估
    var v: i8 = (l->get(): i8)
    if v > 100
        alert->set((1: @), 0)
    return false
...
### 21.4 `map` —— 依赖图（源 → 目标）+ 编译期环检测

`map` 关键字（上下文标识）在「源」与「目标」之间架一条**显式有向边**，令 token 构成**依赖关系图**：

- **源**（`map` 前）：触发 / 上游，门按其状态开合、其变更触发本 `dep`；
- **目标**（`map` 后）：输出 / 下游，由 follow 写入，**自身不触发本 dep**（杜绝自环回灌）。
- 源与目标都注入局部名糖（`toks` = 源 ++ 目标）。块形态 `map` 单独成行作分隔，行内形态 `… map o:"id"`。
- **编译期环检测**：全单元所有 `map` 边汇成有向图，编译器三色 DFS 检测——出现环（含自环）即编译期
  报错，要求依赖图为 **DAG**。普通 `dep`（无 `map`）不贡献图边。
...
### 21.5 烘焙常量：依赖图度量（O(1) 查询方法）

`map` 把 token 连成 DAG 后，编译器在编译期对图跑一组算法（拓扑 / 最长路 / 关键路径 / 传递闭包 /
支配树 / Tarjan SCC），把每个 token 的图属性算成**字面量常量**烘焙进生成的 C，运行时只是 **O(1)
查表**（类比游戏的 lightmap / navmesh 烘焙：图是静态的，故所有结构度量都能预计算）。由此 token 句柄
暴露一组查询方法（不引入新语法，同 `get` / `set`）：

| 方法 | 含义 |
|------|------|
| `t.depth()`       | 依赖图深度（源=0，最长路 = 第几级 / 第几层） |
| `t.critical()` / `t.slack()` | 是否在**关键路径**（最长链）上 / **松弛余量**（0=瓶颈，旁支有余量） |
| `t.fanin()` / `t.fanout()` | 扇入度（被多少上游依赖）/ 扇出度（驱动多少下游；高=枢纽 / 广播源） |
| `t.reach()`       | 可达下游数：变更后须重算的下游总数（脏标记影响范围 / 失效爆炸半径） |
| `t.batch()` / `t.batch_width()` | 拓扑波次编号（同波两两无路径，可并行）/ 本波并行宽度 |
| `t.checkpoint()` / `t.dom_size()` | 是否为**支配咽喉 / 缓存边界** / 支配子树规模 |

> 图算法本体住在 `builtins/tok/graph`（`graph.h` 契约 + `graph_impl.c` 实现，编译进 `scc`），
> 编译器只做「AST 边 → 整数图」适配与「结果 → C 字面量」烘焙；运行时（`tok_impl.c`）只持常量并以
> getter 返回。详见 [builtins/tok.md](builtins/tok.md) §1.3.2。示例：`examples/feature51.sc`（关键
> 路径）/ `feature52`（扇入扇出）/ `feature53`（可达性）/ `feature54`（拓扑分批）/ `feature55`
> （支配树）。

### 21.6 `loop` —— 受控反馈环（dep loop）

`map` 强制 DAG（环即报错），但有些问题**天生有环**：迭代求解（牛顿法、不动点）、控制反馈、互递归
收敛。`loop` 是与 `map` **正交**的边拓扑——占 `map` 的位置（块式 `loop` 单独成行 / 行内 `… loop t:"id"`）
声明一条**反馈边**，**豁免环检测**，编译期对全部 `loop` 边跑 Tarjan 求**强连通分量（SCC）**，烘焙每
token 的「反馈簇编号 + 簇大小」。

| 维度 | `map`（DAG 边） | `loop`（反馈边） |
|------|----------------|------------------|
| 环 | 编译期报错（强制 DAG） | 豁免；Tarjan 缩点 |
| 触发 | 源变更**自动级联** | 源**不自动级联**（杜绝无限环），仅经显式 `t.loop_run(max)` 按簇驱动 |
| 烘焙 | depth / critical / fanin… | `scc()` 簇编号 + `scc_size()` 簇大小 |

- **门逻辑正交**：`all` / `any`（与门 / 或门）与边拓扑（none / `map` / `loop`）相互独立——`loop` 不改
  门语义。
- 方法：`t.scc()`（反馈簇编号，非反馈=0）/ `t.scc_size()`（簇大小，>1 或含自环=反馈）/ `t.loop_run(max)`
  （驱动 `t` 所在簇迭代至多 `max` 轮，返回实际轮数；非反馈簇不迭代）。示例 `examples/feature50.sc`
  （2 节点反馈簇牛顿法整数平方根，从 a=100 收敛到 a=b=10=√100）。

### 21.7 `back` —— 反向遍历（反向传播骨架）

`back` 是硬关键字（语句位）：自一个输出 token 出发，沿反向邻接（`producers[]`，由 `map` 目标反挂）
**按反拓扑序**（深者先行）逐个以 `acting=TOK_BACK` 唤起上游 `dep` 的 `follow`——follow 体内判
`this->active == TOK_BACK` 即走**反向计算**（读目标、写源，如梯度反传）。

```sc
back loss, (1: @)                   # 自 loss 反向遍历，先灌梯度种子 1（seed 可省）
```

形态：`back <token 句柄>[, <种子>]`。种子非空则先灌入起点（不触发前向级联），随后反向 BFS。编译期
已保证 `map` 图为 DAG（环检测），反向遍历必终止。follow 返回**非 0** 即**提前中止**本轮遍历（返回
0 = 走完全程，向后兼容），供 drain 拉取式协作层「处理一个最深节点后停扫重扫」。

**drain 驱动 ≠ form input 驱动**：`back` / `TOK_BACK` 是与前向推送（`set` 变更 / `form` 就绪）**正交
的拉取驱动**，但**正交点在 combine、不在 form**。一套纯 drain 流水线**无需写 combine 体**（每帧独立
...
token 句柄是 `token&`，用箭头 `->` 调方法：`t->get()`（返回 `@`，调用点 `(e: T@)` / `(e: i8)` 还原）/
`t->set(v, tag)`（`v` 为 `@`、`tag` 为 `i4`；`set` 触发依赖级联）。

- **边界（v2）**：模块域静态对象，不支持 `@` 导出（注册延迟到模块 `init` / `main` 序章）；id 为进程内
  字符串键（adt 哈希 dict intern）；`'/'` 前缀 id 走每-token 细粒度无锁多线程（seqlock 读 + 每-dep
  自旋门）。
- 协议 `@def token` 及上下文 `@def __sctok_in` / `@def __scdep_in` 声明在 `builtins/op.sc`；C ABI 与
  运行时为独立模块 `builtins/tok/`（`tok.h` + `tok_impl.c`，经 op→tok 隐式依赖始终随工程链接）。
...
- 作用域退出钩子 `final`：在当前作用域所有退出路径（落出/`return`/`break`/`continue`）执行清理块，多个 `final` 后进先出（LIFO），与 `T@` 协作时先于自动拆边执行（详见 §15.7）
...
- io 内置关键字：`stringify(值[, 缓存, 大小])` 按静态类型 JSON 格式化（返回 `string` 或缓存 `char&`，需 `inc adt.sc`，转 C 时生成独立 `stringify.h`）；`print` 日志输出函数（拼接糖 + C printf 兼容模式，F/E/W/I/D/V 级别、SC_LOG 过滤，需 `inc io.sc`，详见 §15.6）
...
**开发工具：**
- 独立语言服务器协议（LSP）实现：把现有 VS Code 扩展的语言能力（诊断/悬停/补全/跳转/引用/重命名/符号/调用层次）抽取为标准 LSP server，复用到 Neovim/Emacs/其它编辑器（VS Code 内已通过原生 provider 全量实现）
...
- 条件编译（预处理能力）是**有意不提供**的设计决策而非缺失，见第 5 节“无预处理器设计”。
- 部分 warning 类诊断**有意不做**（与「零误报」原则冲突）：未使用局部变量/参数、影子变量等（sc 语料中大量合法写法会触发）；自赋值检测（`x = x` 常被用作打断点调试的占位语句，并非笔误）。
- **C 侧声明目前只支持函数与宏**：C 实现接口 `@fnc name::` / `fnc name::`（见 §9「与 C 互通」）与 C 宏桥接 `def name::`（见 §18.2）能把 C 侧的**函数 / 宏**引入 sc 命名域；但 C 侧的**类型**（结构 `struct` / 联合 `union` / 枚举 `enum` / `typedef`）暂**无法**在 sc 中声明或暴露。若确需以某个 C 类型作字段/局部（如内联持有平台句柄），只能用 `::TypeName` 原样逃逸到 C 域（例如 mt 模块 `h: ::mutex_t`）——它布局精确、可当字段用，但**不是** sc 的一等类型：不可在 sc 侧对其成员寻址、方法调用或参与 sc 类型推导。因此这类 C 平台类型无需（也不应）加 `sc_` 前缀去「兼容 sc 透传」——sc 本就无法把它当作声明来暴露。

### 25.1 不提供拷贝构造 / 移动构造（有意为之）

sc **没有**拷贝构造、移动构造、拷贝赋值运算符这类「值语义钩子」——值的复制只有一种语义：**逐字节按值拷贝**，且只对 **C 原语裸结构体**（不递归内嵌自动指针 `T@` 的纯值聚合）开放。这**不是**对结构体复制的一刀切禁止——纯值聚合照常自由复制；**只有当一个按值聚合（递归）内嵌了智能指针（自动指针 `T@`）时**，对它的按值复制才会**编译期报错**。判据是**有条件**的（见下「精确触发判据」）。

**会报错**的复制点（仅当类型递归内嵌 `T@`）：

| 场景 | 示例（`Holder` 含 `T@` 字段） | 报错要点 |
|------|------|---------|
| 以初值拷贝构造 | `var g: Holder = h` | 「禁止以初值拷贝构造内嵌自动指针 T@ 的按值聚合」 |
| 整体赋值（`=`） | `a = b`（`a`、`b` 同为该聚合） | 「禁止整体赋值内嵌自动指针 T@ 的按值聚合」 |
| 按值传参（非导出函数） | `fnc f: i4, x: Holder` | 「按值传参会复制胖指针字节而不更新引用记账，请改用 & 引用传参」 |
| 按值返回（非导出函数） | `fnc f: Holder` | 「按值返回……请改用 T@（移交所有权）或 & 引用返回」 |
| 跨 C ABI（`@fnc` 导出 / `rpc` / C 实现接口） | 参数或返回为 `T@` 或内嵌 `T@` 的聚合 | 「跨传输/跨 C ABI 无法维护胖指针引用图与 ARC」 |

**照常放行**的复制点（不内嵌 `T@`，或本身就是 `T@`）：

```sc
@def Point: { x: i4, y: i4 }      # 纯值聚合，无 T@
var b: Point = a                  # ✓ 通过：值即全部，自由复制
var arr[3]: Point = grid          # ✓ 通过：定长值数组复制

var p: node@ = q                  # ✓ 通过：直接 T@ 字段/变量走 bind 记账（拷贝即绑定，见 §5.1）
```

**精确触发判据**（与编译器 `valueAggrEmbedsFat` 实现一致）：当且仅当某类型是「**按值聚合**」——即 `ptr==0` 且非数组、且其本身不是 `T@`——**且**递归内嵌至少一个 `T@` 成员（直接 `T@` 字段，或其按值聚合 / 内联结构字段递归含 `T@`，带环检测）时，上述按值复制点报错。反之**放行**：

- 直接 `T@` 字段本身的 `=` / 以 `T@`、`T()`、`nil` 初始化——走 bind/move 引用记账（拷贝即绑定/移动，见 §5.1）；
- 指针 / 数组的复制——拷贝的是地址而非内嵌胖指针字节；
- 不含任何 `T@` 的纯值聚合（全标量 / 定长数组等）——可自由按值复制赋值。

**为什么有意如此（设计思想）。** 数据对象本就分两类：

1. **没有唯一身份的数据块**——其值即其全部，适合自由复制赋值（标量、纯值结构、定长数组）；
2. **具有唯一身份的逻辑对象**——身份不可复制，适合堆分配 + 引用共享（`T@` 自动指针 / `T&` 裸引用）。声明此类对象的专用语法是**堆专属结构体 `def Name&`**（见 §10.2.1）：它从源头取消 `Name` 值类型，使「按值赋值」根本无从写起。

拷贝构造 / 移动构造是用「一套技术上的万能公式」去套用这两种**本质不同**的对象：对数据块是冗余仪式，对逻辑对象则会把「身份」也一并复制，制造出两个自称同一身份的副本，混乱且极易出错（双重释放、悬垂、引用计数失衡）。sc 因此不设这条万能公式，而是让两类对象各走各的路——值类型直接复制，逻辑对象经 `T@` 绑定 / `&` 借用共享，编译器在边界处强制二者不混用。这不是「缺特性」，而是把 C++ 里靠程序员自律维持的约定**前移到语言规则**里。
