...
- 生成的 C 代码主要面向 GCC/Clang。**容器下标糖** `t[key]`（`find` 命中取元素）当前依赖 GNU 语句表达式 `({ ... })`，MSVC 不支持 → 该特性暂不可在 MSVC 上编译。其余启动/退出钩子已经由 `platform.h` 的 `SC_CONSTRUCTOR`/`SC_DESTRUCTOR` 宏做了 MSVC 适配（`.CRT$XCU` 段 + `atexit`），线程局部、对齐等亦有可移植宏；唯下标糖的语句表达式尚无平凡可移植等价，需把临时量提到语句上下文后才能消除（核心改动，单列）。
