...
### 5.3 半自动指针 `T@&`（单例指针：裸指针 + 退域自动释放）

`T@&` 是介于裸指针 `T&` 与自动指针 `T@` 之间的第三条路径：**物理上就是裸指针 `T*`**
...
var p:   tensor@&    # 单例指针：裸地址 + 退域自动销毁
var a:   tensor@     # 自动指针：24 字节胖指针，参与引用图共享
```

**赋值来源受限（独占语义）。** `T@&` 只接受三类右值：取址 `&x`、`nil`、或**堆构造/工厂/方法**
返回的新对象（如 `zeros(...)`、`p->reshape(...)`、数组取出 `(out[0]: tensor&)`）。它不做引用图
共享——把同一个对象同时塞进两个 `T@&` 会在退域时双重释放，由你负责保证每个 `T@&` 独占一个对象。

```sc
@fnc main: i4
    var p: tensor@& = zeros(2, shp, DT_F4)   # 持有一个新箱子
    p->fill(1.0)
