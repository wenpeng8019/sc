...
>
> 远期规划支持 `--cstd=c99|c11|c17` 显式选择输出档位（见 §16），以适配老旧或受限工具链。
...
- `def`：定义类型、枚举、结构体、联合体、类型别名、宏（见 §5「结构化宏」）。
- `var`：定义变量。
- `let`：定义常量。
- `tls`：定义线程局部变量（static 存储期，见 9.1）。
- `fnc`：定义函数、函数类型、方法。
- `rpc`：定义伪形参函数（参数/返回值展开为同名结构体，见 8.8）。
- `mix`：展开一个 `def` 宏（顶层展开声明，函数体内展开语句，见 §5「结构化宏」）。
- `inc`：引入头文件或 sc 模块。
...
- **语义层**：根的 `@` 导出声明以 `external` 并入每个递归依赖单元，使根的全局
  类型/操作在依赖单元的 sc 源码中**直接可见**（无需 `inc` 根模块）。
...
@fnc app_report: void, m: metric           # 全局通用操作
    printf("[report] %s = %d\n", m.tag, m.value)
fnc main: i4
...
它不是新关键字，而是把已有直觉（`@fnc name::`、`fnc T::m` 表示 C 实现接口）
统一延伸为一条贯穿规则，用来表达 sc 普通语法**够不着**的 C 互通需求。
...
> 表达式位置能求值的 C 函数式宏直接写名调用即可（见上文「直接用 C 名」），无需 `mix`；
> `mix` 专门覆盖「展开后产生声明/语句」这类普通 sc 语法够不着的宏。宏体内的名字不参与
> sc 符号解析（形参没有 sc 类型），因此宏展开生成的符号对 sc 不可见——若要在 sc 代码里
> 按名引用宏生成的全局，需再用 `let/var X:: T` 认领。
...
`inc stdio.h`、`inc "args.h"` 引入的是**别处定义**的外部 C 头，转 C 为
`#include <stdio.h>` / `#include "args.h"`。sc 不解析其内容（除可选 `--clang` 描述符
...
| 互通形式 | 方向 | 对应的 `.h` | 谁定义符号 |
|---|---|---|---|
| 直接写 C 名 `name(...)` / `name` | 入站（调用/取用） | **B 依赖导入** | 被导入的头（如 `stdio.h`、`args.h`） |
| `fnc f::` | 出站（声明接口） | **A 导出生成** | 本模块配套 `_impl.c` |
| `let/var X:: T` | 出站（声明符号） | **A 导出生成** | 本模块配套 `_impl.c`，或下游 `mix` 展开 |
...
> `.h` 直接取用，就直接写名 `X` 而非再 `let X:: T`，否则即为重复声明。它的另一典型用途是认领由
> **宏在调用点现场展开**出来、没有任何头文件声明的全局——例如某模块 `inc env.sc`（连带得到
> `args.h`）后写 `mix ARGS_B(abc, ...)`，该宏内部定义出全局 `ARGS_abc`，于是用
> `let ARGS_abc:: arg_var_st` 把它登记进 sc（落地 `extern arg_var_st ARGS_abc;`，与宏的定义对齐）。
> 这种「宏现场定义 + `::` 认领」全部发生在**下游使用方模块**里，而非提供宏的 `env.sc`/`args.h` 本身。
...
`@` 前缀表示导出对象，`--emit-c` 时会额外生成头文件声明（即上文 §5「导出生成的 `.h`」）；
...
> `::` 在声明侧（`fnc name::`、`fnc T::m`）表示「C 实现的接口」，在表达式/绑定侧
> （`::name(...)`、`let X:: T`）表示「调用/认领 C 侧名字」——同一接缝符的统一延伸，
> 详见 §5「`::` —— sc↔C 映射符」。
...
- 初始化列表：数组用 `[1, 2, 3]`、结构体/联合体用 `{1, 2}` 或指定成员 `{a = 1, b = 2}`，可嵌套（见 10.2）
...
- 结构体、联合体、枚举、类型别名
- 结构化宏：对象宏 `def NAME: = value` 与函数宏 `def name: p... \n <体>`（可变参 `...` → `__VA_ARGS__`、粘贴 `\` → `##`、串化 `` `x` `` → `#`）映射到 C `#define`；`mix name(args)` 顶层展开声明 / 函数体内展开语句
...
- io 内置关键字：`stringify(值[, 缓存, 大小])` 按静态类型 JSON 格式化（返回 `string` 或缓存 `char&`，需 `inc adt.sc`，转 C 时生成独立 `stringify.h`）；`print` 日志输出函数（拼接糖 + C printf 兼容模式，F/E/W/I/D/V 级别、SC_LOG 过滤，需 `inc io.sc`，详见 §11.4）
...
**开发工具：**
- 独立语言服务器协议（LSP）实现：把现有 VS Code 扩展的语言能力（诊断/悬停/补全/跳转/引用/重命名/符号/调用层次）抽取为标准 LSP server，复用到 Neovim/Emacs/其它编辑器（VS Code 内已通过原生 provider 全量实现，见 §15）
