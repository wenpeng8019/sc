# nn 自动微分引擎回归用例（define-by-run）：
#   - val/tape/backward 引擎 + matmul/add(广播)/relu 反向；
#   - linear 模块（Kaiming 初始化）+ mse_loss + SGD；
#   - 单样本回归 2 → 4(relu) → 2，多轮训练后做一次推理；
#   - 末尾 nn_tape_clear() + 各对象 drop()（drop 为析构器，盒子由编译器回收）。
inc ts.sc
inc nn.sc
