...
uint32_t _sc_gfx_pool_alloc(_sc_gfx_pool* p) {
...
void _sc_gfx_pool_release(_sc_gfx_pool* p, uint32_t id) {
    int index = _sc_gfx_slot_index(id);
...
#define LOOKUP_IMPL(name, poolfield, arrfield, type)                          \
    type* _sc_gfx_lookup_##name(uint32_t id) {                                \
        if (!id) return NULL;                                                 \
        int index = _sc_gfx_slot_index(id);                                   \
        if (index <= 0 || index >= _sc_gfx.poolfield.size) return NULL;       \
        type* res = &_sc_gfx.arrfield[index];                                 \
        if (res->slot.id != id) return NULL;                                  \
        return res;                                                           \
    }

LOOKUP_IMPL(buffer,   buffer_pool,   buffers,   _sc_gfx_buffer_t)
LOOKUP_IMPL(image,    image_pool,    images,    _sc_gfx_image_t)
LOOKUP_IMPL(sampler,  sampler_pool,  samplers,  _sc_gfx_sampler_t)
LOOKUP_IMPL(shader,   shader_pool,   shaders,   _sc_gfx_shader_t)
LOOKUP_IMPL(pipeline, pipeline_pool, pipelines, _sc_gfx_pipeline_t)
...
static const _sc_gfx_backend_api* pickBackend(void) {
    switch ((sc_gpu_backend)sc_gpu_query_backend()) {
        case SC_GPU_BACKEND_METAL:
#ifdef SC_GPU_METAL
            return _sc_gfx_backend_metal();
#else
            return NULL;
#endif
        case SC_GPU_BACKEND_GL:
#ifdef SC_GPU_GL
            return _sc_gfx_backend_gl();
#else
            return NULL;
#endif
        case SC_GPU_BACKEND_NULL:
        default:
            return _sc_gfx_backend_null();
    }
}
...
int sc_gfx_init(const sc_gfx_desc* desc) {
    if (_sc_gfx.valid) { _sc_gfx_log("init: 已初始化"); return 0; }
    if (!sc_gpu_isvalid()) { _sc_gfx_log("init: 须先 sc_gpu_init"); return 0; }
    memset(&_sc_gfx, 0, sizeof(_sc_gfx));
    _sc_gfx.desc = desc ? *desc : (sc_gfx_desc){0};
    _sc_gfx.desc.buffer_pool_size   = DEF(_sc_gfx.desc.buffer_pool_size, 128);
    _sc_gfx.desc.image_pool_size    = DEF(_sc_gfx.desc.image_pool_size, 128);
    _sc_gfx.desc.sampler_pool_size  = DEF(_sc_gfx.desc.sampler_pool_size, 64);
    _sc_gfx.desc.shader_pool_size   = DEF(_sc_gfx.desc.shader_pool_size, 64);
    _sc_gfx.desc.pipeline_pool_size = DEF(_sc_gfx.desc.pipeline_pool_size, 64);

    const _sc_gfx_backend_api* api = pickBackend();
    if (!api) { _sc_gfx_log("init: gpu 后端对应的 gfx 翻译未编入本库"); return 0; }

    _sc_gfx_pool_init(&_sc_gfx.buffer_pool,   _sc_gfx.desc.buffer_pool_size);
    _sc_gfx_pool_init(&_sc_gfx.image_pool,    _sc_gfx.desc.image_pool_size);
    _sc_gfx_pool_init(&_sc_gfx.sampler_pool,  _sc_gfx.desc.sampler_pool_size);
    _sc_gfx_pool_init(&_sc_gfx.shader_pool,   _sc_gfx.desc.shader_pool_size);
    _sc_gfx_pool_init(&_sc_gfx.pipeline_pool, _sc_gfx.desc.pipeline_pool_size);
    _sc_gfx.buffers   = (_sc_gfx_buffer_t*)  calloc((size_t)_sc_gfx.buffer_pool.size,   sizeof(_sc_gfx_buffer_t));
    _sc_gfx.images    = (_sc_gfx_image_t*)   calloc((size_t)_sc_gfx.image_pool.size,    sizeof(_sc_gfx_image_t));
    _sc_gfx.samplers  = (_sc_gfx_sampler_t*) calloc((size_t)_sc_gfx.sampler_pool.size,  sizeof(_sc_gfx_sampler_t));
    _sc_gfx.shaders   = (_sc_gfx_shader_t*)  calloc((size_t)_sc_gfx.shader_pool.size,   sizeof(_sc_gfx_shader_t));
    _sc_gfx.pipelines = (_sc_gfx_pipeline_t*)calloc((size_t)_sc_gfx.pipeline_pool.size, sizeof(_sc_gfx_pipeline_t));

    if (!api->init(&_sc_gfx.desc)) {
        _sc_gfx_log("init: 后端 %s 初始化失败", api->name);
        sc_gfx_shutdown();
        return 0;
    }
    _sc_gfx.api = api;
...
void sc_gfx_shutdown(void) {
    if (_sc_gfx.api) {
        /* 逐资源销毁（后端仍活着时） */
        for (int i = 1; i < _sc_gfx.pipeline_pool.size; i++)
            if (_sc_gfx.pipelines[i].slot.state == _SC_GFX_SLOT_VALID)
                _sc_gfx.api->pipeline_destroy(&_sc_gfx.pipelines[i]);
        for (int i = 1; i < _sc_gfx.shader_pool.size; i++)
            if (_sc_gfx.shaders[i].slot.state == _SC_GFX_SLOT_VALID)
                _sc_gfx.api->shader_destroy(&_sc_gfx.shaders[i]);
        for (int i = 1; i < _sc_gfx.sampler_pool.size; i++)
            if (_sc_gfx.samplers[i].slot.state == _SC_GFX_SLOT_VALID)
                _sc_gfx.api->sampler_destroy(&_sc_gfx.samplers[i]);
        for (int i = 1; i < _sc_gfx.image_pool.size; i++)
            if (_sc_gfx.images[i].slot.state == _SC_GFX_SLOT_VALID)
                _sc_gfx.api->image_destroy(&_sc_gfx.images[i]);
        for (int i = 1; i < _sc_gfx.buffer_pool.size; i++)
            if (_sc_gfx.buffers[i].slot.state == _SC_GFX_SLOT_VALID)
                _sc_gfx.api->buffer_destroy(&_sc_gfx.buffers[i]);
        _sc_gfx.api->shutdown();
    }
    free(_sc_gfx.buffers);   free(_sc_gfx.images); free(_sc_gfx.samplers);
    free(_sc_gfx.shaders);   free(_sc_gfx.pipelines);
    _sc_gfx_pool_free(&_sc_gfx.buffer_pool);
    _sc_gfx_pool_free(&_sc_gfx.image_pool);
    _sc_gfx_pool_free(&_sc_gfx.sampler_pool);
    _sc_gfx_pool_free(&_sc_gfx.shader_pool);
    _sc_gfx_pool_free(&_sc_gfx.pipeline_pool);
    memset(&_sc_gfx, 0, sizeof(_sc_gfx));
}
...
sc_gfx_pixelformat_info sc_gfx_query_pixelformat(sc_gpu_pixel_format fmt) {
    ...
    if (_sc_gfx.valid && _sc_gfx.api->query_pixelformat)
        _sc_gfx.api->query_pixelformat(fmt, &info);
    return info;
}
...
sc_gfx_buffer sc_gfx_make_buffer(const sc_gfx_buffer_desc* desc) {
    ...
    uint32_t id = _sc_gfx_pool_alloc(&_sc_gfx.buffer_pool);
    if (!id) { _sc_gfx_log("make_buffer: 池满"); return 0; }
    _sc_gfx_buffer_t* buf = &_sc_gfx.buffers[_sc_gfx_slot_index(id)];
    memset(buf, 0, sizeof(*buf));
    buf->slot.id = id;
    buf->desc = *desc;
    resolveBufferDesc(&buf->desc);
    if (buf->desc.usage == SC_GFX_USAGE_IMMUTABLE && !buf->desc.data.ptr) {
        _sc_gfx_log("make_buffer: IMMUTABLE 须提供初始数据");
        buf->slot.state = _SC_GFX_SLOT_FAILED;
    } else {
        buf->slot.state = _sc_gfx.api->buffer_create(buf) ? _SC_GFX_SLOT_VALID
                                                          : _SC_GFX_SLOT_FAILED;
    }
    return id;
}
...
sc_gfx_image sc_gfx_make_image(const sc_gfx_image_desc* desc) {
    ...
    img->slot.state = _sc_gfx.api->image_create(img) ? _SC_GFX_SLOT_VALID
                                                     : _SC_GFX_SLOT_FAILED;
    return id;
}
...
sc_gfx_sampler sc_gfx_make_sampler(const sc_gfx_sampler_desc* desc) {
    if (!_sc_gfx.valid || !desc) return 0;
    uint32_t id = _sc_gfx_pool_alloc(&_sc_gfx.sampler_pool);
    if (!id) { _sc_gfx_log("make_sampler: 池满"); return 0; }
    _sc_gfx_sampler_t* smp = &_sc_gfx.samplers[_sc_gfx_slot_index(id)];
    memset(smp, 0, sizeof(*smp));
    smp->slot.id = id;
    smp->desc = *desc;
    resolveSamplerDesc(&smp->desc);
    smp->slot.state = _sc_gfx.api->sampler_create(smp) ? _SC_GFX_SLOT_VALID
                                                       : _SC_GFX_SLOT_FAILED;
    return id;
}
...
sc_gfx_shader sc_gfx_make_shader(const sc_gfx_shader_desc* desc) {
    if (!_sc_gfx.valid || !desc) return 0;
    uint32_t id = _sc_gfx_pool_alloc(&_sc_gfx.shader_pool);
    if (!id) { _sc_gfx_log("make_shader: 池满"); return 0; }
    _sc_gfx_shader_t* shd = &_sc_gfx.shaders[_sc_gfx_slot_index(id)];
    ...
    if (desc->reflect_json) {
        if (!_sc_gfx_parse_reflect(desc->reflect_json, &shd->reflect)) {
            _sc_gfx_log("make_shader: 反射清单解析失败");
            shd->slot.state = _SC_GFX_SLOT_FAILED;
            return id;
        }
    } else {
        _sc_gfx_reflect* r = &shd->reflect;
        for (int i = 0; i < SC_GFX_MAX_UNIFORM_BLOCKS * 2; i++) {
            const sc_gfx_uniform_block_desc* ub = &desc->uniform_blocks[i];
            if (ub->size == 0) continue;
            _sc_gfx_reflect_block* b = &r->blocks[r->block_count++];
        ...
        for (int i = 0; i < SC_GFX_MAX_IMAGES; i++) {
            ...
            if (!is->name) continue;
            _sc_gfx_reflect_sampler* s = &r->samplers[r->sampler_count++];
            s->stage = is->stage; s->slot = is->slot;
            strncpy(s->name, is->name, sizeof(s->name) - 1);
        }
    }
    shd->slot.state = _sc_gfx.api->shader_create(shd, desc) ? _SC_GFX_SLOT_VALID
                                                            : _SC_GFX_SLOT_FAILED;
    return id;
}
...
sc_gfx_pipeline sc_gfx_make_pipeline(const sc_gfx_pipeline_desc* desc) {
    if (!_sc_gfx.valid || !desc) return 0;
    _sc_gfx_shader_t* shd = _sc_gfx_lookup_shader(desc->shader);
    if (!shd || shd->slot.state != _SC_GFX_SLOT_VALID) {
        _sc_gfx_log("make_pipeline: 无效 shader");
        return 0;
    }
    uint32_t id = _sc_gfx_pool_alloc(&_sc_gfx.pipeline_pool);
    if (!id) { _sc_gfx_log("make_pipeline: 池满"); return 0; }
    _sc_gfx_pipeline_t* pip = &_sc_gfx.pipelines[_sc_gfx_slot_index(id)];
    memset(pip, 0, sizeof(*pip));
    pip->slot.id = id;
    pip->desc = *desc;
    pip->shader = shd;
    resolvePipelineDesc(&pip->desc);
    pip->slot.state = _sc_gfx.api->pipeline_create(pip) ? _SC_GFX_SLOT_VALID
                                                        : _SC_GFX_SLOT_FAILED;
    return id;
}
...
#define DESTROY_IMPL(name, poolfield, type, apifn)                             \
    void sc_gfx_destroy_##name(type hnd) {                                     \
        if (!_sc_gfx.valid) return;                                            \
        _sc_gfx_##name##_t* res = _sc_gfx_lookup_##name(hnd);                  \
        if (!res) return;                                                      \
        if (res->slot.state == _SC_GFX_SLOT_VALID) _sc_gfx.api->apifn(res);    \
        res->slot.id = 0;                                                      \
        res->slot.state = _SC_GFX_SLOT_FREE;                                   \
        _sc_gfx_pool_release(&_sc_gfx.poolfield, hnd);                         \
    }
...
void sc_gfx_update_buffer(sc_gfx_buffer hnd, const sc_gfx_range* data) {
    if (!_sc_gfx.valid || !data || !data->ptr) return;
    _sc_gfx_buffer_t* buf = _sc_gfx_lookup_buffer(hnd);
    if (!buf || buf->slot.state != _SC_GFX_SLOT_VALID) return;
    if (buf->desc.usage == SC_GFX_USAGE_IMMUTABLE) {
        _sc_gfx_log("update_buffer: IMMUTABLE 不可更新"); return;
    }
    if (data->size > buf->desc.size) {
        _sc_gfx_log("update_buffer: 数据超出缓冲大小"); return;
    }
    _sc_gfx.api->buffer_update(buf, data, 0);
}

int sc_gfx_append_buffer(sc_gfx_buffer hnd, const sc_gfx_range* data) {
    if (!_sc_gfx.valid || !data || !data->ptr) return 0;
    _sc_gfx_buffer_t* buf = _sc_gfx_lookup_buffer(hnd);
    ...
    if ((size_t)offset + data->size > buf->desc.size) {
        ...
        _sc_gfx_log("append_buffer: 溢出");
        return offset;
    }
    _sc_gfx.api->buffer_update(buf, data, offset);
    buf->append_pos = offset + (int)data->size;
    return offset;
}

void sc_gfx_update_image(sc_gfx_image hnd, const sc_gfx_image_data* data) {
    if (!_sc_gfx.valid || !data) return;
    _sc_gfx_image_t* img = _sc_gfx_lookup_image(hnd);
    if (!img || img->slot.state != _SC_GFX_SLOT_VALID) return;
    if (img->desc.usage == SC_GFX_USAGE_IMMUTABLE) {
        _sc_gfx_log("update_image: IMMUTABLE 不可更新"); return;
    }
    _sc_gfx.api->image_update(img, data);
}

/* ---- 状态查询 --------------------------------------------- */

static int slotToState(_sc_gfx_slot_state s) {
    switch (s) {
...
void sc_gfx_begin_pass(const sc_gfx_pass* pass) {
    if (!_sc_gfx.valid || !pass) return;
    if (_sc_gfx.in_pass) { _sc_gfx_log("begin_pass: pass 嵌套"); return; }

    sc_gfx_pass p = *pass;
    _sc_gfx_image_t* colors[SC_GFX_MAX_COLOR_ATTACHMENTS] = {0};
    _sc_gfx_image_t* resolves[SC_GFX_MAX_COLOR_ATTACHMENTS] = {0};
    bool has_resolve[SC_GFX_MAX_COLOR_ATTACHMENTS] = {0};
    _sc_gfx_image_t* depth = NULL;
    int color_count = 0;
    for (int i = 0; i < SC_GFX_MAX_COLOR_ATTACHMENTS; i++) {
        if (!p.colors[i].image) break;
        colors[i] = _sc_gfx_lookup_image(p.colors[i].image);
        if (!colors[i] || colors[i]->slot.state != _SC_GFX_SLOT_VALID) {
            _sc_gfx_log("begin_pass: 无效颜色附件 %d", i); return;
        }
        if (p.resolves[i].image) {
            resolves[i] = _sc_gfx_lookup_image(p.resolves[i].image);
            if (!resolves[i] || resolves[i]->slot.state != _SC_GFX_SLOT_VALID) {
                _sc_gfx_log("begin_pass: 无效 resolve 附件 %d", i); return;
            }
            has_resolve[i] = true;
        }
        color_count++;
    ...
    if (p.depth_stencil.image) {
        depth = _sc_gfx_lookup_image(p.depth_stencil.image);
        if (!depth || depth->slot.state != _SC_GFX_SLOT_VALID) {
            _sc_gfx_log("begin_pass: 无效深度附件"); return;
        }
    }
    _sc_gfx.pass_is_swapchain = (color_count == 0 && !p.compute);
    if (_sc_gfx.pass_is_swapchain && !sc_gpu_query_current_surface()) {
        _sc_gfx_log("begin_pass: gpu 无当前 surface（先 sc_gpu_make_current）"); return;
    }
    /* 交换链 MSAA：后端自行解析到 drawable，store 默认 RESOLVE */
    bool sc_resolve[SC_GFX_MAX_COLOR_ATTACHMENTS] = {0};
    if (_sc_gfx.pass_is_swapchain) {
        sc_gpu_surface_desc sd;
        if (sc_gpu_query_surface_info(0, &sd) && sd.sample_count > 1)
            sc_resolve[0] = true;
    }
    resolvePassAction(&p.action, _sc_gfx.pass_is_swapchain ? 1 : color_count,
                      _sc_gfx.pass_is_swapchain ? sc_resolve : has_resolve);
    _sc_gfx.in_pass = true;
    _sc_gfx.cur_pipeline = NULL;
    _sc_gfx.api->begin_pass(&p, colors, color_count, resolves, depth);
}

void sc_gfx_apply_viewport(int x, int y, int w, int h, int origin_top_left) {
    if (!_sc_gfx.valid || !_sc_gfx.in_pass) return;
    _sc_gfx.api->apply_viewport(x, y, w, h, origin_top_left != 0);
}

void sc_gfx_apply_scissor(int x, int y, int w, int h, int origin_top_left) {
    if (!_sc_gfx.valid || !_sc_gfx.in_pass) return;
    _sc_gfx.api->apply_scissor(x, y, w, h, origin_top_left != 0);
}

void sc_gfx_apply_pipeline(sc_gfx_pipeline hnd) {
    if (!_sc_gfx.valid || !_sc_gfx.in_pass) return;
    _sc_gfx_pipeline_t* pip = _sc_gfx_lookup_pipeline(hnd);
    if (!pip || pip->slot.state != _SC_GFX_SLOT_VALID) {
        _sc_gfx_log("apply_pipeline: 无效管线"); return;
    }
    _sc_gfx.cur_pipeline = pip;
    _sc_gfx.api->apply_pipeline(pip);
}

void sc_gfx_apply_bindings(const sc_gfx_bindings* bnd) {
    if (!_sc_gfx.valid || !_sc_gfx.in_pass || !bnd) return;
    if (!_sc_gfx.cur_pipeline) { _sc_gfx_log("apply_bindings: 未 apply_pipeline"); return; }

    _sc_gfx_buffer_t* vbufs[SC_GFX_MAX_VERTEX_BUFFERS] = {0};
    _sc_gfx_buffer_t* ibuf = NULL;
    /* [0]=vs [1]=fs [2]=cs */
    _sc_gfx_image_t*   imgs[3][SC_GFX_MAX_IMAGES] = {{0}};
    _sc_gfx_sampler_t* smps[3][SC_GFX_MAX_SAMPLERS] = {{0}};
    _sc_gfx_buffer_t*  sbufs[3][SC_GFX_MAX_STORAGE_BUFFERS] = {{0}};

    for (int i = 0; i < SC_GFX_MAX_VERTEX_BUFFERS; i++) {
        if (!bnd->vertex_buffers[i]) continue;
        vbufs[i] = _sc_gfx_lookup_buffer(bnd->vertex_buffers[i]);
        if (!vbufs[i]) { _sc_gfx_log("apply_bindings: 无效顶点缓冲 %d", i); return; }
    }
    if (bnd->index_buffer) {
        ibuf = _sc_gfx_lookup_buffer(bnd->index_buffer);
        if (!ibuf) { _sc_gfx_log("apply_bindings: 无效索引缓冲"); return; }
    }
    const sc_gfx_stage_bindings* stages[3] = { &bnd->vs, &bnd->fs, &bnd->cs };
    for (int s = 0; s < 3; s++) {
        for (int i = 0; i < SC_GFX_MAX_IMAGES; i++)
            if (stages[s]->images[i])
                imgs[s][i] = _sc_gfx_lookup_image(stages[s]->images[i]);
        for (int i = 0; i < SC_GFX_MAX_SAMPLERS; i++)
            if (stages[s]->samplers[i])
                smps[s][i] = _sc_gfx_lookup_sampler(stages[s]->samplers[i]);
        for (int i = 0; i < SC_GFX_MAX_STORAGE_BUFFERS; i++)
            if (stages[s]->storage_buffers[i])
                sbufs[s][i] = _sc_gfx_lookup_buffer(stages[s]->storage_buffers[i]);
    }
    _sc_gfx.api->apply_bindings(_sc_gfx.cur_pipeline, bnd, vbufs, ibuf, imgs, smps, sbufs);
}

void sc_gfx_apply_uniforms(int stage, int slot, const void* data, uint64_t size) {
    if (!_sc_gfx.valid || !_sc_gfx.in_pass || !data || !size) return;
    _sc_gfx.api->apply_uniforms(stage, slot, data, (size_t)size);
}

void sc_gfx_draw(int base_element, int element_count, int instance_count) {
    if (!_sc_gfx.valid || !_sc_gfx.in_pass || !_sc_gfx.cur_pipeline) return;
    if (instance_count <= 0) instance_count = 1;
    _sc_gfx.api->draw(base_element, element_count, instance_count);
}

void sc_gfx_dispatch(int gx, int gy, int gz) {
    if (!_sc_gfx.valid || !_sc_gfx.in_pass || !_sc_gfx.cur_pipeline) return;
    if (!_sc_gfx.cur_pipeline->desc.compute) {
        _sc_gfx_log("dispatch: 当前非计算管线"); return;
    }
    _sc_gfx.api->dispatch(gx, gy, gz);
}

void sc_gfx_end_pass(void) {
    if (!_sc_gfx.valid || !_sc_gfx.in_pass) return;
    _sc_gfx.api->end_pass();
    _sc_gfx.in_pass = false;
    _sc_gfx.cur_pipeline = NULL;
}

void sc_gfx_commit(void) {
    if (!_sc_gfx.valid) return;
    _sc_gfx.api->commit();   /* 后端末尾调 sc_gpu_frame_end() */
    _sc_gfx.frame_index++;
    /* 每帧重置 append 位置（stream 语义） */
    for (int i = 1; i < _sc_gfx.buffer_pool.size; i++) {
        _sc_gfx_buffer_t* b = &_sc_gfx.buffers[i];
