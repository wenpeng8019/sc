...
case "$TOOLCHAIN" in
    gnu)
        compile_gnu "surface.c"
        ;;
    msvc)
        compile_msvc "surface.c"
        ;;
esac

case "$TOOLCHAIN" in
    msvc)
        "$AR" /nologo /out:"libsurface.$TARGET.a" $ALL_OBJS
        ;;
    gnu)
        "$AR" rcs "libsurface.$TARGET.a" $ALL_OBJS
        ;;
esac

rm -rf "$OBJ_DIR"
echo "  -> libsurface.$TARGET.a"

if [[ $IS_HOST -eq 1 ]]; then
    rm -f libsurface.a
    ln -sf "libsurface.$TARGET.a" libsurface.a 2>/dev/null || cp "libsurface.$TARGET.a" libsurface.a
    echo "  -> libsurface.a -> libsurface.$TARGET.a"
fi
