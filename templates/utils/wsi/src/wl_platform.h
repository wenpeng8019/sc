...
void _glfwSetWindowMonitorWayland(window_st* window, monitor_st* monitor, int xpos, int ypos, int width, int height, int refreshRate);
...
void _glfwSetCursorPosWayland(window_st* window, double xpos, double ypos);
...
bool _glfwCreateCursorWayland(cursor_st* cursor, const GLFWimage* image, int xhot, int yhot);
...
void _glfwGetMonitorWorkareaWayland(monitor_st* monitor, int* xpos, int* ypos, int* width, int* height);
GLFWvidmode* _glfwGetVideoModesWayland(monitor_st* monitor, int* count);
