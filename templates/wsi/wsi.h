...
/*! @brief 内存分配回调函数指针类型。
 *
 *  这是内存分配回调的函数指针类型。内存分配回调函数具有以下签名：
 *  @code
 *  void* function_name(size_t size, void* user)
 *  @endcode
 *
 *  此函数必须返回一个至少 `size` 字节长的内存块，如果分配失败则返回 `NULL`。
 *  请注意，并非 GLFW 的所有部分都能优雅地处理分配失败。
 *
 *  此函数必须支持在 @ref sc_wsi_init 期间（库尚未标记为已初始化时）被调用，
 *  以及在 @ref sc_wsi_terminate 期间（库不再标记为已初始化时）被调用。
 *
 *  通过此函数分配的任何内存，将在库终止或更早的时候通过同一个分配器释放。
 *
 *  通过此函数分配的任何内存必须对任何对象类型都适当地对齐。
 *  如果使用 C99 或更早版本，此对齐方式取决于平台，但与 `malloc` 提供的一致。
 *  如果使用 C11 或更高版本，则为 `alignof(max_align_t)` 的值。
 *
 *  大小将始终大于零。大小为零的分配在到达自定义分配器之前会被过滤掉。
 *
 *  如果此函数返回 `NULL`，GLFW 将发出 @ref SC_WSI_ERR_OUT_OF_MEMORY。
 *
 *  此函数不得调用任何 GLFW 函数。
 *
 *  @param[in] size 内存块的最小大小（以字节为单位）。
 *  @param[in] user 来自分配器的用户自定义指针。
 *  @return 新分配的内存块地址，如果发生错误则返回 `NULL`。
 *
 *  @pointer_lifetime 返回的内存块必须至少在被释放之前保持有效。
 *
 *  @reentrancy 此函数不应调用任何 GLFW 函数。
 *
 *  @thread_safety 此函数必须支持从调用 GLFW 函数的任何线程被调用。
 *
 *  @ingroup init
 */
 ...
 *
 *  通过此函数分配的任何内存必须对任何对象类型都适当地对齐。
 *  如果使用 C99 或更早版本，此对齐方式取决于平台，但与 `realloc` 提供的一致。
 *  如果使用 C11 或更高版本，则为 `alignof(max_align_t)` 的值。
 *
 *  块地址永远不会是 `NULL`，且大小将始终大于零。
 *  将块重新分配为零大小会在到达自定义分配器之前转换为释放操作。
 *  将 `NULL` 重新分配为非零大小会在到达自定义分配器之前转换为常规分配操作。
 *
 *  如果此函数返回 `NULL`，GLFW 将发出 @ref SC_WSI_ERR_OUT_OF_MEMORY。
 *
 *  此函数不得调用任何 GLFW 函数。
 *
 *  @param[in] block 要重新分配的内存块地址。
 *  @param[in] size 内存块新的最小大小（以字节为单位）。
 *  @param[in] user 来自分配器的用户自定义指针。
 *  @return 新分配或调整大小后的内存块地址，如果发生错误则返回 `NULL`。
 *
 *  @pointer_lifetime 返回的内存块必须至少在被释放之前保持有效。
 *
 *  @reentrancy 此函数不应调用任何 GLFW 函数。
 ...
 *  @endcode
 *
 *  此函数可以释放指定的内存块。此内存块是通过同一个分配器分配的。
 *
 *  此函数必须支持在 @ref sc_wsi_init 期间（库尚未标记为已初始化时）被调用，
 *  以及在 @ref sc_wsi_terminate 期间（库不再标记为已初始化时）被调用。
 *
 *  块地址永远不会是 `NULL`。对 `NULL` 的释放操作在到达自定义分配器之前会被过滤掉。
 *
 *  如果此函数返回 `NULL`，GLFW 将发出 @ref SC_WSI_ERR_OUT_OF_MEMORY。
 *
 *  此函数不得调用任何 GLFW 函数。
 *
 *  @param[in] block 要释放的内存块地址。
 *  @param[in] user 来自分配器的用户自定义指针。
 *
 *  @pointer_lifetime 调用此函数后，GLFW 将不再访问指定的内存块。
 *
 *  @reentrancy 此函数不应调用任何 GLFW 函数。
 *
 *  @thread_safety 此函数必须支持从调用 GLFW 函数的任何线程被调用。
 ...
 *  to @ref sc_wsi_init_allocator before initializing the library.
 *
 *  @ingroup init
 */
 ...
 *  SC_WSI_ERR_INVALID_VALUE and the init allocator will be unchanged.
 *
 ...
 *
 *  @errors Possible errors include @ref SC_WSI_ERR_INVALID_VALUE.
 *
