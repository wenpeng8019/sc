...
独立神经元模块 [`neuron`](../utils/)（`inc neuron.sc`）。
...
hello-dnn/hello.sc        本工程：tok/dep 拓扑 + 数据集 + 训练循环 + main
    inc neuron.sc         neuron 侧车（含 forward/backward/sgd/adam/seed_mse 方法）+ 激活库 + edge_step + 损失
```

`hello.sc` 标 `@@`（可执行根、同目录唯一的 `main`）；`inc neuron.sc` 把内置 `neuron` 模块的导出件并入
（`neuron` = 独立神经元基元，原 utils 三件套已升格为此内置模块，见 [../utils/README.md](../utils/README.md)）。
...
| 前向算子 | follow `else` 分支 | 调 `n->forward`（`z = b + Σ wᵢ·xᵢ` → 激活）→ `pulse` 触发级联 |
| 反向算子（自动微分） | follow `TOK_BACK` 分支 | 调 `n->backward`：`gz = grad·act'(pre)`；`∂L/∂wᵢ += gz·xᵢ`，并把 `wᵢ·gz` 累加回上游 grad |
...
1. **加神经元/层**：加 `tok` + 一条 `dep` + 侧车槽；dep 体只需一行 `edge_step(this, 下游)` + `return false`
   （`edge_step` 自动按前向/反向分流），复制即可。`depth()/batch()/critical()` 会自动重新烘焙——网络变化无需手改调度。
2. **新激活 / 新优化器 / 新算子**：改内置 [`neuron`](../utils/)（`builtins/neuron/neuron.sc`）即可，本工程不动（见 utils/README）。
