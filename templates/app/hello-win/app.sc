# hello-win —— sc 桌面 app 脚手架（Windows / Win32）
#
# 最小的原生窗口程序：打开窗口、跑主循环、响应输入、干净退出。
# 拷走本目录改名，即可作为你自己 Windows app 的起点。
#
# 桌面平台的循环由「程序自己」掌控（显式 while）；iOS/Android 的循环由系统
# 框架驱动，形态不同——见 templates/app/README.md。
#
# 用法（在 macOS/Linux 上交叉编译出 hello.exe，需 mingw-w64）：
...
fnc main: i4
    ...
    print "wsi ", major, ".", minor, ".", rev, " · 桌面 app 脚手架 (Windows)\n"

    # ---- 创建并显示窗口 ----
    var win: ::sc_window& = wsi_win_create(960, 540, "sc · hello-win", nil, nil)
    if win == nil
        print "窗口创建失败\n"
        wsi_app_cleanup()
        return 1
