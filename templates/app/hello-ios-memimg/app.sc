# hello-ios-memimg —— iOS(Metal) memimg 无表面渲染验证 + 窗口三角形（path A 回调驱动）
#
# gpu_headless_demo「Mode A」的 iOS 等效：先在 on_after_startup 建 MEMORY surface
# （内存交换链，Metal 底层 = IOSurface）→ 渲染三角形 → dequeue 取帧 → memimg_map
# 映射 CPU（IOSurfaceLock/GetBaseAddress）→ 读中心/角落像素并打印校验，验证 iOS 上
# Metal memimg（IOSurface 回读，与已验的 macOS 路径同源）端到端可用；自检完成并
# gpu_shutdown 后，再在 on_main_window_created 于 UIKit 窗口重建 gpu/gfx，on_frame
# 每帧画同一三角形——屏幕可见（不再是黑屏），一眼即知渲染链路活着。
#
# 用法（M 芯片 Mac + iOS 模拟器）：
#   ./templates/app/hello-ios-memimg/build.sh
#   校验：控制台看 "memimg 自检" 各行——center 应为三角形色、corner 应为深蓝底
#     BGRA≈76/25/12，末行 "Metal memimg 回读链路端到端 OK"；屏幕显示彩色三角形。
...
# 主窗口创建：memimg 自检已在 on_after_startup 完成并 gpu_shutdown，此处于 UIKit
# 窗口重建 gpu/gfx/管线（native_window = wsi 交付的 UIView*），供 on_frame 每帧渲染。
fnc on_main_window_created: win: ::sc_window&
    var fbw: i4 = 0
    var fbh: i4 = 0
    wsi_win_get_framebuffer_size(win, &fbw, &fbh)
    print "hello-ios-memimg: 窗口就绪 · 帧缓冲 ", fbw, " x ", fbh, "（自检完毕，转窗口三角形）\n".
    ...
    if gpu_init(&gd) == 0
        print "hello-ios-memimg: 窗口 gpu_init 失败\n".
        return

    var fd: ::sc_gfx_desc
    ::memset(&fd, 0, sizeof(::sc_gfx_desc))
    if gfx_init(&fd) == 0
        print "hello-ios-memimg: 窗口 gfx_init 失败\n".
        gpu_shutdown()
        return
    ...
    var shd: u4 = gfx_make_shader(&sd)
    if shd == 0
        print "hello-ios-memimg: 窗口 make_shader 失败\n".
        return
    g_shd = shd
    ...
    var pip: u4 = gfx_make_pipeline(&pd)
    if pip == 0
        print "hello-ios-memimg: 窗口 make_pipeline 失败\n".
        return
    g_pip   = pip
    g_ready = 1
    print "hello-ios-memimg: 窗口三角形管线就绪\n".

# 每帧回调（CADisplayLink 驱动）：清屏（深蓝底）+ 画三角形。
fnc on_frame:
    if g_ready == 0
        return
    g_frames = g_frames + 1
    ...
    if g_frames % 60 == 0
        print "hello-ios-memimg: 窗口帧 ", g_frames, "\n".
