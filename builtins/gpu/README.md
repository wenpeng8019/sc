...
无需预构建：`inc gpu.sc` 即用（`add src/*` 源码动态编译，后端宏由
src/internal.h 按目标自推导 + 本目录 [.sc](.sc) 段配置注入）。
模块库按需：

```sh
../../compiler/build/scc . --build                 # → libgpu.a
../../compiler/build/scc . --build --target <档>   # → libgpu.<suffix|triple>.a
```

macOS 链接需系统框架（本目录 [.sc](.sc) 配置文件已带，编译器自动注入）：
...
gpu.sc                sc FFI 描述（add src/* 动态编译交付）
.sc                   模块构建/链接配置（按目标段注入，见 compiler.md §7.4）
src/internal.h        env 后端 vtable、surface/memimg 池、环调度（含后端宏自推导）
