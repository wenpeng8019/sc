...
bool     tensor_copy_from(tensor *_this, tensor *o);       /* 形状须一致，逐元素拷入（dtype 自动转换） */
...
bool     tensor_scatter_(tensor *_this, int32_t axis, tensor *index, tensor *src); /* 沿 axis 按 index 原地写入 src */
...
bool     tensor_add_(tensor *_this, tensor *o);
bool     tensor_sub_(tensor *_this, tensor *o);
bool     tensor_mul_(tensor *_this, tensor *o);
bool     tensor_div_(tensor *_this, tensor *o);
bool     tensor_add_scalar_(tensor *_this, double s);
bool     tensor_sub_scalar_(tensor *_this, double s);
bool     tensor_mul_scalar_(tensor *_this, double s);
bool     tensor_div_scalar_(tensor *_this, double s);

/* ---- 规约（axis<0 → 全规约得标量张量；否则沿 axis 降维；keepdim!=0 保留长度 1 的维） ---- */
tensor  *tensor_sum(tensor *_this, int32_t axis, bool keepdim);
tensor  *tensor_mean(tensor *_this, int32_t axis, bool keepdim);
tensor  *tensor_prod(tensor *_this, int32_t axis, bool keepdim);
tensor  *tensor_max(tensor *_this, int32_t axis, bool keepdim);
tensor  *tensor_min(tensor *_this, int32_t axis, bool keepdim);
tensor  *tensor_argmax(tensor *_this, int32_t axis, bool keepdim);       /* 结果 dtype=I8 */
tensor  *tensor_argmin(tensor *_this, int32_t axis, bool keepdim);       /* 结果 dtype=I8 */
tensor  *tensor_std(tensor *_this, int32_t axis, bool keepdim);          /* 总体标准差（除 N） */
tensor  *tensor_var(tensor *_this, int32_t axis, bool keepdim);          /* 总体方差（除 N） */
...
tensor  *tensor_any(tensor *_this, int32_t axis, bool keepdim);          /* → DT_BOOL */
tensor  *tensor_all(tensor *_this, int32_t axis, bool keepdim);          /* → DT_BOOL */
tensor  *tensor_median(tensor *_this, int32_t axis, bool keepdim);       /* 沿 axis 中位数 */
tensor  *tensor_percentile(tensor *_this, double q, int32_t axis, bool keepdim); /* 第 q 百分位 */
...
bool     tensor_qr(tensor *_this, void *out);              /* QR；out=tensor*[2]=(Q,R) */
bool     tensor_eigh(tensor *_this, void *out);            /* 对称特征（Jacobi）；out=(vals,vecs) */
bool     tensor_svd(tensor *_this, void *out);             /* SVD；out=tensor*[3]=(U,S,V) */
...
bool     tensor_relu_(tensor *_this);                      /* 原地 relu */
...
tensor  *tensor_dropout(tensor *_this, double p, bool train); /* 训练随机置零，推理直通 */
tensor  *tensor_sdpa(tensor *_this, tensor *k, tensor *v, bool causal); /* scaled dot-product attention */
...
/* ---- 比较/杂项 ---- */
bool     tensor_equal(tensor *_this, tensor *o);                       /* 形状+逐元素全等 */
bool     tensor_allclose(tensor *_this, tensor *o, double rtol, double atol); /* 近似相等 */
bool     tensor_shuffle_(tensor *_this);                   /* 沿首维随机洗牌（原地） */
bool     tensor_save(tensor *_this, const char *path);     /* 存为 NumPy .npy（C-order） */
void     tensor_print(tensor *_this);                      /* 形状 + 元素（大张量省略） */

/* ---- 构造补充 / 随机 / IO（自由函数） ---- */
tensor  *tri(int32_t n, int32_t m, int32_t k, int32_t dtype); /* [n,m] 下三角 1 */
bool     meshgrid(void *arr, int32_t n, int32_t indexing, void *out); /* N 个 1D→N 个 N 维网格 */
