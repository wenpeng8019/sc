 ...
 */
#define P_MT_IMPL       /* opt-in 展开 platform.h 线程层，取用 <pthread.h> 的 pthread_key_t TLS（每线程堆的线程退出析构） */
#include "platform.h"
#include "mem.h"
...
void *sc_chunk(uint64_t size) {
    size_t s = (size_t)size;
    if (s == 0) s = 1;
    return (s > SC_MEM_SMALL_MAX) ? mem_alloc_large(s) : mem_alloc_small(s);
}

void *sc_chunk0(uint64_t size) {
    void *p = sc_chunk(size);
    if (p && size) memset(p, 0, (size_t)size);
    return p;
}

void *sc_chunk_array(uint64_t count, uint64_t size) {
    if (count != 0 && size > (uint64_t)SIZE_MAX / count) return NULL;  /* count*size 溢出 */
    return sc_chunk0(count * size);                                       /* 0 由 chunk0 视为 1 */
}

void *sc_chunk_aligned(uint64_t size, uint64_t align) {
    size_t a = (size_t)align;
    if (a <= MEM_ALIGN) return sc_chunk(size);          /* 默认返回值已 16 对齐 */
...
void *sc_refit(void *p, uint64_t size) {
    if (!p) return sc_chunk(size);
    if (size == 0) { sc_recycle(p); return NULL; }
    mem_block *b = (mem_block*)((uint8_t*)p - MEM_HDR);
    ...
    if (b->owner == MEM_LARGE_OWNER) {
        ...
        mem_tail_check(b, p);
        size_t oldu = b->info;
        if (want > SC_MEM_SMALL_MAX) {                            /* 大→大：原地 realloc */
...
void sc_mem_stat(sc_mem_stat_t *out) {
    if (!out) return;
...
void sc_arena_init(sc_arena *_this, uint64_t cap) {
...
void *sc_arena_chunk(sc_arena *_this, uint64_t size) {
    arena_impl *ai = (arena_impl*)_this->h;
    if (!ai) return NULL;
...
void sc_arena_reset(sc_arena *_this) {
    arena_impl *ai = (arena_impl*)_this->h;
    if (!ai) return;
    arena_blk *b = ai->head;
    if (!b) return;
...
void sc_arena_drop(sc_arena *_this) {
    arena_impl *ai = (arena_impl*)_this->h;
    if (!ai) return;
    arena_blk *b = ai->head;
...
bool sc_shm_make(sc_shm *self, const char *name, uint64_t size, uint32_t flags) {
    if (!self || !name) return 0;
    self->h = NULL;

    int rdonly = (flags & SC_SHM_RDONLY) != 0;
    int excl   = (flags & SC_SHM_EXCL) != 0;
