...
| sync | `i4, fn, params, prio: i4, delay_ms: i8` | 阻塞带回复（编译器经 `sync work(args), q` 派发）：投递后阻塞至别的消费者执行完，结果回填 `params` 首字段返回槽 `_`；返回 0 成功 / -1 已关闭或失败。同线程 sync 到自身会死锁；finite timeout/死锁替代待后续 |
...
delay=0），带优先级/延迟的即发即忘用 `async<...>` 丢弃返回的 promise。finite timeout /
循环死锁替代为后续架构升级阶段。详见 syntax.md §15.2 与 `examples/feature42.sc`。
