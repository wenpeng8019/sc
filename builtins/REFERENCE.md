...
| sync | `i4, fn, params, psize, prio: i4, delay_ms: i8, timeout_ms: i8` | 阻塞带回复（编译器经 `sync work(args), q` 派发）：投递后阻塞至别的消费者执行完，结果回填 `params` 首字段返回槽 `_`；`timeout_ms<=0` 无限等 / `>0` 最多等 N 毫秒（P5c）。返回 0 成功 / 1 超时 / -1 已关闭或失败（可经可选状态出参 `sync<timeout:ms> w(), q, &st` 取回）。有限超时走堆盒子+引计数（执行方只碰盒子堆缓冲，调用方超时不 UAF）。同线程 sync 到自身会死锁；循环死锁替代待后续 |
