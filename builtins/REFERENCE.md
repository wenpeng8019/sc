...
| 成员 | 签名 | 说明 |
|------|------|------|
| post | `bool, fn, params, psize` | 投递一条 rpc 消息（编译器经 `<<` 派发，见 op.h） |
| sync | `i4, fn, params` | 阻塞带回复（编译器经 `sync work(args), q` 派发）：投递后阻塞至别的消费者执行完，结果回填 `params` 首字段返回槽 `_`；返回 0 成功 / -1 已关闭或失败。同线程 sync 到自身会死锁；超时/优先级/死锁替代待后续 |
| async | `promise&, fn, params, psize` | 非阻塞带回复（编译器经 `async work(args), q` 派发）：投递后立即返回 `promise&`，消费者执行完后兑现，调用方 `p->wait()` 阻塞取结果；参数缓冲堆分配由 promise 拥有。返回 NULL=投递失败 |
