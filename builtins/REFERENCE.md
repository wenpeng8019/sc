...
`string()`），不依赖 io 实现：编译器按实参的静态类型合成格式化
函数（需 `inc adt.sc`）。转 C 时按类型生成的静态内联格式化器写入独立的
`stringify.h`，由生成的 `.c` 在类型定义之后 `#include`。两种形态按实参个数静态派发：

```sc
var s: string = stringify(t)       # 返回 string（JSON 文本），调用方负责 drop
print("t=%s", s.cstr())
s.drop()

var b[256]: char
print("t=%s", stringify(t, b, 256)) # 在给定缓存内构建（截断保证 NUL 结尾），
