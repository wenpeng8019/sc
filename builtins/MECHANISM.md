...
`com` 是 sc 语言层的**设备通讯端点**，采用「协议对象」模式（vtable 方法指针），
与 `mt` 的 pool/queue 同构。核心：**编译器只认识 `<<` / `>>` 收发糖与六种展开模式**，
设备如何读写完全由用户实现的 `read`/`write`/`readable`/`writable` 方法指针决定——编译器不内置任何设备知识。

```sc
def com: <limit> {           # 容器类型：以 limit 为注入节点
    dev: &                    # 设备专属不透明句柄
    rq: ioq&                  # 读队列（nil=不支持异步读）
    wq: ioq&                  # 写队列（nil=不支持异步写）

    fnc alloc: limit&, size: u4, ending: &  # 分身构造（com[...] 有界读句柄）
    fnc free: s: limit&                     # 分身析构
    fnc read: ret, data: &, size: u4&       # 设备读（每对象 io 实现）
    fnc write: ret, buf: &, size: u4&       # 设备写
    fnc readable: ret, id: &&               # 读就绪查询（多路复用探测）
    fnc writable: ret, id: &&               # 写就绪查询
    fnc close: ret                          # 关闭设备
    fnc error: ret                          # 错误回调
}
```
...
语法层面 `<<` / `>>` 是链式结合的收发操作符：`com << a << b`、`com >> x >> y`。
最左操作数须为 `com` 基类型，逐个展开执行。编译器按**所在函数类型（同步/异步）× 右操作数形态**，
展开为六种模式：

| 模式 | 函数类型 | 右操作数 | 展开 |
|------|---------|---------|------|
| **A** | 同步 fnc | 普通变量 `v` | `base.write(&base, &v, &sz)` — 直接收发 sizeof(v) 字节 |
| **B** | 同步 fnc | com[...] 句柄 `s` | `limit_read(&base, s._)` — 框架读循环（仅 >>） |
| **C** | 同步 fnc | rpc 调用 `rpc(args)` | 逐字段序列化 rpc 参数，字段顺序 = 收发协议 |
| **D** | 异步 rpc | 普通变量 `v` | `com_read/write_async` + `future_await` 切点 |
| **E** | 异步 rpc | com[...] 句柄 `s` | `com_limit_read_async` 包裹框架循环为 future |
| **F** | 异步 rpc | rpc 调用/名 | 逐字段异步收发 + 状态机恢复 + 收毕触发 worker |

**模式 A·B·C（同步）**是最简路径：fnc 内直接调用 com 的方法指针完成读写。
**模式 D·E·F（异步）**是最复杂路径：rpc 含 `await` 时编译为状态机，每个收发点是
一个切点——发起异步 io 返回 future，若未就绪则保存 stage 并 `return` 让出 CPU；
就绪后事件循环 resume 状态机继续。

### 6.3 框架读循环（limit_read）

`com[...]` 句柄是 `com` 的分身/切片，语法 `var s: com[size, ending]` + `s = com`。
它的 `limit` 头只有两个每对象方法接口 + 两个字段：

| 成员 | 类型 | 含义 |
|------|------|------|
| `size` | `u4` | 定长大小 或 每次最大 chunk |
| `len` | `u4` | 框架回写：已累计读取计数 |
| `data()` | MethodPtr | 返回缓冲基址（用户实现） |
| `ending()` | MethodPtr | 动态截止判定，≥0 命中、<0 继续读（可 nil） |

框架 `limit_read` 是**最小内核**——策略全在用户的 `data()`/`ending()` 里，框架只跑不变的循环：
`com.read` → 累加 `len` → 若 `ending` 命中则结束 → 否则继续读满或遇 eof。例：HTTP 以 `\r\n\r\n`
截止由用户在 `ending` 里扫描，框架不写死任何协议。

### 6.4 多路复用就绪探测

`readable`/`writable` 统一两种设备就绪通知方式：
- `*id != nil`（fd/句柄）→ **支持多路复用**：注册进 epoll/kqueue/IOCP/poll，O(1) 就绪通知
- `*id == nil` → **仅能轮询**：看返回值（<0 报错 / 0 pending / 1 ready）

两条路径收敛到同一结果：「设备就绪 → 执行 io → future_done 兑现」。

### 6.5 ioq 循环缓冲

`ioq` 是 com 的读写缓存队列——自动膨胀的循环缓冲。每个 item 为连续一组值：
`[size, buf]`（io 缓冲）或 `[0, cb, data]`（完成回调）。`com` 提供 `rq`/`wq`（非 nil）
即标记自身支持异步 io。

### 6.6 异步 io 事件循环

`async_io` 专司「设备 io 就绪」层，与 `async_loop`（future<ID> 消息派发/协程 resume）正交：
登记了待办 io 的 com → 探测就绪（readable/writable）→ ioq.pull 取队头 →
执行 com.read/write → future_done 兑现 → 接回 await 状态机。

### 6.7 架构分层

```
op.sc（声明层·默认导入）
  └─ com / limit / ioq 数据结构 + 方法协议
op.h（C ABI 层·默认带入）
  └─ struct com / struct limit / struct ioq + extern 原型
op_impl.c（通用运行时·始终链接）
  └─ limit_read 框架循环 + com_read/write_async + async_io 多路复用循环
用户设备实现（应用层·可选模块如 builtins/io/）
  └─ 提供 read/write/readable/writable/close/error 方法指针
```

编译器只做**语法糖展开**（`<<`/`>>` → 六种模式之一调用 com 方法），设备逻辑与编译器彻底解耦。
...
| ts | — | — | — | — | 无（纯库模块） |
| com | — | — | `<<`/`>>` → 六种模式展开 | — | `emitComChain`/`emitComAwait`/`emitComRpc*` |
