...
- **裸→胖**：禁止隐式转换（裸指针无 tar/own 信息）；显式 `(e: @)`/`(e: *)` 允许，但**按标量装箱**（见下，非托管，不入引用图）
...
- **标量/裸指针入自动指针（显式擦除装箱）**：`(e: @)`/`(e: *)` 把任意非胖源装入空记账句柄——整数经 `(intptr_t)` 擦入 `.p`、裸指针/数组直取 `.p`，`tar=NULL`（不计入边）、`dtor=NULL`（不析构）、`own=SC_OWN_RAW`（不计出边，仅 `@`/`sc_afat` 有）。纯值搬运，非托管：不参与引用图、解绑 no-op；调用点经 `(e: T@)`/`(e: T*)`/`(e: i8)` 还原。tok 共享量值即用此装箱（整数触发标量、裸指针句柄无堆分配地塞入值槽）
...
## 2. 内存安全检查 `--check=mem` / `--check=ptr`

> 以下两个检查开关独立于 T@ 自动指针机制，属于语言级的运行时内存安全守卫。

### 2.1 `--check=mem`：栈/堆缓冲区越界探测

在编译时对函数内栈数组和 `--check=mem` 标注的堆分配注入尾哨兵（canary）。栈数组超额分配额外空间存放魔数哨兵值，域退出时校验——若哨兵被破坏（数组越界写），立即报告并定位源码。同时注入 `-fstack-protector-strong` 保护返回地址。堆对象在 `sc_ref` 头尾各加一个 canary，`free` 时校验。

### 2.2 `--check=ptr`：nil 解引用与数组下标越界

编译时在指针解引用（`p->field`、`*p`）前注入 nil 校验——命中即 abort 并报告源码位置。对编译期已知维度的栈数组，在下标访问时注入越界校验（`i >= 0 && i < len`），命中即 abort 并报告下标与数组长度。

---

## 3. 类机制 `cls` / `dim`：分派器 + 维度

> **机制框架**
> - 每个 `cls` 对象首部注入一个分派函数指针 `_class`，
> - 所有 `dim` 折叠进唯一分派器的 `switch` 分支（→ §4.1）
> - 维度名是全局选择子——同名 `dim` 跨类即同一消息，语义检查强制参数签名一致（→ §4.1、§2.5）
> - `tril` 三态基础类型 + `object` 类型擦除引用 + `instanceOf` O(1) 身份判定（→ §4.2）
> - 五个保留维度（CLS_ID/REF/DROP/OBJ_KEY/OBJ_NAME/RLT_KEY/RLT_NAME）与用户 dim 选择子从 7 起（→ §4.3）
> - `_class` 在四种构造点自动安装（栈/堆/T@/全局），跨单元 class.h 聚合保证编号一致（→ §4.4、§2.5）
