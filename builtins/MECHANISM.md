...
### 6.1 采用 vtable「协议对象」模式，实现设备自定义扩展和实现

`com` 结构体通过 C 侧的函数指针成员（MethodPtr），实现具体设备 I/O 的抽象。sc 侧用 `fnc name:`（单冒号、无函数体）声明这些槽，编译器将其编译为结构体中的函数指针字段，而非独立函数。C ABI 落在 `op.h`：

```c
// op.h:466-478 — com 的 C ABI（由 sc 侧 def com: <limit> { ... } 编译生成）
struct com {
    limit *(*alloc)(com *_this, uint32_t size, void *ending);  // 分身构造
    void  (*free)(com *_this, limit *s);                        // 分身析构
    int32_t (*read)(com *_this, void *data, uint32_t *size);    // 设备读（*size in/out，回写实际字节数）
    int32_t (*write)(com *_this, void *buf, uint32_t *size);    // 设备写
    int32_t (*error)(com *_this);
    int32_t (*readable)(com *_this, void **id);   // 读就绪探测（*id=可监听句柄 或 nil→看返回值）
    int32_t (*writable)(com *_this, void **id);   // 写就绪探测
    int32_t (*close)(com *_this);
    void   *dev;                  // 设备专属不透明句柄（如 FILE* / socket fd）
    ioq    *rq;                   // 读缓冲队列（nil=不支持异步读）
    ioq    *wq;                   // 写缓冲队列（nil=不支持异步写）
    limit  *_self;                // <limit> 容器注入：分身回指本体（见 §6.3）
};
```

关键设计：`read` / `write` / `readable` / `writable` 均为**每对象方法指针**——同一 `com` 类型的不同设备实例可以指向不同的实现函数（文件设备走 `fread`/`fwrite` + `fileno` 多路复用，socket 设备走 `recv`/`send`，管道走 `read`/`write`）。编译器只认 `com` 类型名来识别 `<<`/`>>` 链（见 §6.2），不内置任何设备逻辑。
...
### 6.3 通过 sc 的分身/切片能力，实现有界数据的读取

com 的分身/切片能力基于 sc 的 **`<limit>` 容器机制**。`com` 声明为 `def com: <limit>`（见 §6.1 的 C ABI），编译器在 `com` 结构体首部注入 `limit _self` 节点，`com` 通过 `alloc`/`free` 方法指针管理 `limit` 分身的创建与销毁。

**句柄语法**：`var s: com[size, ending]` 是 sc 的 `T[...]` 分身/切片句柄——方括号内存放分身的构造参数，展开为一个匿名结构体 `com__project { ..., _: limit& }`。`s = com` 赋值触发分身构造：

```c
// 编译器展开（codegen）：
s._ = com.alloc(&com, size, ending);  // 调用 com 的 alloc 方法指针，传入 size + ending 透传构造
s._->_self = &com;                    // 回写回指指针：分身 → 本体（<limit> 容器机制注入）
```

`limit` 节点承载有界读视图，只暴露两个字段 + 两个方法指针：

```sc
// op.sc — limit 的类型布局
@def limit: {
    size: u4               # ending=nil: 定长大小；ending!=nil: 每次最大 chunk
    len:  u4               # 框架回写：已累计读取的字节数
    fnc data: &            # 每对象 MethodPtr：返回缓冲基址（用户实现）
    fnc ending: ret        # 每对象 MethodPtr：动态截止判定，≥0=命中（保留值为len），<0=继续
}
```

**分身生命周期**：`s = nil` 触发析构 `com.free(&com, s._); s._ = nil`，释放有界读视图及其关联资源。

至此，编译器完成分身构造与绑定。真正驱动读写的是 **`limit_read`**（`op_impl.c:377`）——一个不变的框架读循环，策略全部外移给 `data()` / `ending()`：

```c
// op_impl.c:377 — limit_read 的实际实现（唯一「框架确定」逻辑）
...
- `ending != nil` → **动态模式**：`size` 只是每次最大 chunk，每读一段回调 `ending(s)` 判定是否命中截止；返回值 ≥0 = 命中（`len` 设为该值），<0 = 继续
- 框架不做缓冲区管理、不做协议解析——HTTP `\r\n\r\n` 截止由用户在 `ending` 里扫描，定长文件读取由用户在 `data()` 里决定缓冲在哪
