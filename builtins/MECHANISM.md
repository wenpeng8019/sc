...
| 方案 | 追踪方向 | 能做的 | 做不到的 |
|------|---------|--------|---------|
| C++ `shared_ptr` | 只记入边 | 无人引用时自动释放目标 | 持有者带着未清理的出边释放了发现不了 |
| Rust | 编译期借用检查 | 全部保证 | 概念门槛高，数据结构很难转成 C |
| **sc `T@`** | **一条边两端各记一次** | 自动释放 + 悬挂和泄露都能在释放点检测 | 不级联释放、不反向置空 |

#### 2.1.1 类型系统：裸指针 vs 胖指针
...
| 能做到 | 做不到 |
|--------|--------|
| 孤立堆对象 ARC 自动释放 | 不级联（不区分所有权层级；靠 `drop` 内的解绑递归回收） |
