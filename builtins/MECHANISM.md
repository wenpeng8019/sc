...
com 是 sc 的 `<limit>` 容器类型（`def com: <limit>`），这意味着 com 同时是**分身/切片的主体**——它可以生成一个有界读视图（`com[...]` 句柄），这在 C 侧展开为注入的 `limit` 结构。

#### 容器注入：`<limit>` 机制的展开

语法 `var s: com[size, ending]` 声明一个 `com[...]` 句柄（`T[...]` 分身语法），然后 `s = com` 把句柄绑定到某个 com 端点：

```sc
var s: com[1024, nil]      # 定长 1024 字节的读缓冲；无 ending（定长模式）
s = file("/tmp/a.bin")     # 绑定到 file com：调 com.alloc 构造 limit 节点
```

编译器展开为两步：

```c
// 1. s = com  →  alloc(&com, size, ending) + _self 回指
s._ = file_com.alloc(&file_com, 1024, NULL);  // 用户实现的 alloc：分配 limit + 缓冲
s._->_self = &file_com;                        // 编译器回写 _self 指向本体 com

// 2. com >> s  →  框架驱动有界读
limit_read(&file_com, s._);                   // 框架确定读循环（见下）
```

`limit` 结构（`op.sc` 声明，`op.h` C ABI）承载一次有界读的全部状态：

| 成员 | 类型 | 含义 |
|------|------|------|
| `size` | `u4` | 定长模式=数据总大小；动态模式=每次最大 chunk |
| `len`  | `u4` | 框架回写：已累计读取的字节数 |
| `data()` | MethodPtr | 每对象方法指针：返回读缓冲基址（用户 `alloc` 里分配，写入起点 `data() + len`） |
| `ending()` | MethodPtr | 每对象方法指针：动态截止判定（可 nil）。每读一段后回调，≥0 = 命中（`len` 设为其值，停止），<0 = 继续读 |

用户只需在 `alloc` 里分配缓冲 + 实现 `data()`（返回基址）和可选的 `ending()`（截止判定），其他交给框架。

#### 技术实现：limit_read — 框架确定读循环
...
两种模式由 `ending` 是否为空自动分派：
- **定长**（`ending == nil`）：读满 `size` 字节即 `IO_EOF`
- **动态**（`ending != nil`）：`size` 只是每次最大 chunk，每读一段回调 `ending(s)`；返回值 ≥0 = 命中，<0 = 继续

框架不内置任何缓冲区管理或协议解析。例：HTTP 以 `\r\n\r\n` 截止——用户只需在 `ending` 里扫描缓冲内容即可，框架只跑这套不变的循环。
