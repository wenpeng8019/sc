...
单例指针**不是句柄结构**——它物理上就是一个普通 `T*`，无任何 16/24/32B 句柄、无引用图记账。唯一的自动化是**退域 RAII**：作用域退出或 `return` 前，若非 nil 则安全执行 `drop`（若有）+ `sc_free`，等同 C++ `unique_ptr`。重新赋值覆盖旧值时也先销毁旧对象。

堆专属指针的析构由 `op.h` 提供的两个内联辅助完成：

```c
sc_ptr_drop(p, drop_fn)
sc_ptr_drop_slot(&p, drop_fn)
```

前者对单个指针做 nil 守卫；后者先把指针槽位置为 `NULL`，再调用用户析构和 `sc_free`。
编译器对可寻址的 `T&` / `T@1` 接收者使用 slot 版本，因此显式 `p->drop()`（兼容
`p.drop()`）与退域 RAII 可以安全汇合：显式析构后 `p` 已经是 nil，退域再次检查时只是
空操作。`T@1` 即使出现显式析构，也不会从退域登记表移除；这是为了保证析构位于未执行的
条件分支时，正常退域仍能回收对象。

slot 通过 `memcpy` 读写而不是把任意 `T**` 强转成 `void**`，避免 C 严格别名规则问题。
析构前置空也让析构回调重入同一所有权槽时不会再次取得已释放对象。

- 必须**具名类型**（`@1` 需具名，类型擦除句柄无意义）
- **单层**：不可再叠 `&`（`T@1` 已是终态）
- 最轻：零句柄开销，适合"唯一持有、退域即释放"的对象
- **显式析构幂等**：可寻址堆指针为 nil 或已显式析构后，再次 `drop` 是空操作；不可寻址
  的临时表达式只能获得 nil 守卫，不能把外部来源置 nil。
- **值对象不套用此规则**：普通 `T` 的 `x.drop()` 仍由类型实现决定是否能重复调用，编译器
  只在退域登记层避免同一显式值析构再次注入，不为任意用户 `drop` 添加运行时状态标志。
...
## 12. 图算法 `graph`：编译期生产者 × 运行时消费者共用一份契约

> 本机制与前 11 章不同：`graph` **不是一个面向用户的语言特性**（没有专属语法糖），而是一个
> **AST 无关的纯算法子模块**——顶点以 `[0, nv)` 整数编号、边以并列数组 `(eu[i] -> ev[i])` 表达。
> 它的特殊性在于**同一份 `graph.h` 契约被三方共用**，其中最关键的是：**编译器 scc 与运行时 builtins
> 引用的是同一份源码**，杜绝「编译期算一套、运行期算另一套」的数据格式漂移。

### 12.1 三重身份：一份契约，三处复用

`builtins/graph/graph.h` 是唯一权威接口（spec interface），`graph_impl.c` 是唯一实现：

| 身份 | 谁 | 怎么用 |
|------|----|--------|
| **编译期生产者** | scc 编译器 | `graph_impl.c` 直接**编译进 scc 二进制**（`compiler/CMakeLists.txt`），`-I builtins` 使 `src/*.cpp` 可 `#include "graph/graph.h"`；编译期对图做预计算 |
| **运行时消费者** | tok 机制运行时 | 生产者算出的图度量**烘焙**为生成代码里的隐藏常量，运行时仅查表；反馈簇（SCC）等结构随之落地 |
| **独立子模块** | 任意 sc 程序 | `graph.sc` 可被 `inc` 当普通算法库直接调用（非 tok 专用） |

**「烘焙」隐喻**：如同游戏引擎把光照预计算成 lightmap、把导航预计算成 navmesh——scc 在编译期把
依赖图的环/拓扑/深度/关键路径等一次算清，结果固化成生成 C 里的字面量常量，运行时零重算（→ §10.2）。
因为生产者与消费者共享同一份 `graph.h`，两端对「顶点编号 / 边数组 / 输出布局」的理解永远一致。

### 12.2 规范接口 API（`sc_graph` + 只读视图）

```c
typedef struct sc_graph { int nv; int ne; const int *eu; const int *ev; } sc_graph;
```

`sc_graph` 是**不可变视图**：调用方持有 `eu`/`ev` 存储，模块只读不接管。全部算法 AST 无关、可复用：

| 函数 | 算法 | 用途 |
|------|------|------|
| `sc_graph_cycle` | 三色 DFS | 有向环检测，回填环路径（`v→…→v` 闭合） |
| `sc_graph_scc` | Tarjan | 强连通分量，簇 >1（或含自环）= 反馈簇 |
| `sc_graph_toposort` / `revtoposort` | Kahn 入度 | 拓扑序 / 逆拓扑序（back 反向传播按此回溯） |
| `sc_graph_depth` | 最长路分层 | 「第几层 / 第几级」深度 |
| `sc_graph_critical` | 正反最长路 | 关键路径 + 松弛余量（流水线瓶颈） |
| `sc_graph_degree` | 计度 | 扇入/扇出（枢纽识别） |
| `sc_graph_reach` | 位集传递闭包 | 脏标记波及面（失效爆炸半径） |
| `sc_graph_batches` | 反链分波 | 并行波次 + 宽度（接 MT 调度） |
| `sc_graph_dominators` | CHK idom | 支配树（检查点 / 缓存边界） |
| `sc_graph_bfs` | 单源 BFS | 无权最短跳数 |
| `sc_graph_components` / `sc_graph_mst` | 并查集 / Kruskal | 弱连通分量 / 最小生成森林 |

### 12.3 scc 构建图的两个场景

编译器在两处「把领域对象翻译成整数图、委托 `graph` 算法、再把结果翻译回领域」——二者**同一套适配范式**（→ §12.4），算法本体都不进编译器源码：

#### 场景一：tok 依赖图环检测（`semantic.cpp::checkTokDepGraphCycles`）

`dep…map` 声明的每条 `dep …: 源 map 目标` 贡献一条 `源→目标` 有向边，全单元汇总成有向图。

- **顶点** = token id（首见序映射为整数）；**边** = `depItems × depTargets` 笛卡尔积。
- **排除**：`external` 声明、`dep loop`（受控反馈边——豁免 DAG 环检测，改走 SCC 烘焙路径）。
- **委托** `sc_graph_cycle`：有环（含自环 `a→a`）→ 编译期报错并打印环路径 `"x" -> "y" -> "x"`，
  因为 follow 写入会回灌触发自身或上游、形成无终止级联。
- 环检测只是入口；**同一张 tok 图**还经 `depth/critical/degree/reach/batches/dominators` 预计算，
  连同 `dep loop` 的 `sc_graph_scc` 反馈簇（簇编号 + 簇大小）一并烘焙为运行时常量（→ §10.2）。

#### 场景二：程序结构依赖图环检测（`proggraph.cpp::findCycles`）

proggraph 把整程序 AST 烘成「结构对象依赖图」（节点 = fnc/rpc/var/method/struct/tok/dep/module…，
边 = call/type/read/write/method/construct/macro/tokdep/import…），用于激活分析与 JSON/HTML 导出。

- **顶点** = 所有**非 `module`** 节点（节点 id 映射为整数）；**边** = 除 `import` 外的全部依赖边。
- **自环**（`a→a`，如自递归函数）单列 `selfLoop` 集合——SCC 会把它归为 size-1 分量，需另行判定。
- **委托** `sc_graph_scc`：簇大小 `>1` = 相互递归环；size-1 且在 `selfLoop` 中 = 自递归环。
  结果写入图导出的 `stats.cycles`（如 `["ping","pong"]`、`["self_rec"]`）。

> 两个场景选用不同算法（场景一要环路径 → `sc_graph_cycle`；场景二要环成员分组 → `sc_graph_scc`），
> 但都是「建整数图 → 调 graph → 翻译结果」，编译器内不重复实现 DFS/Tarjan。

### 12.4 适配范式（adapter pattern）

两处场景（及未来任何 scc 内建图需求）遵循同一四步：

```
① 领域 key（string token id / 节点 id）── 首见序 ──▶ 整数顶点 [0, nv)   （unordered_map<string,int>）
② 遍历领域对象，抽出边表 eu/ev（跳过不相关的边/节点，特殊边如 loop/self 另记）
③ sc_graph g{ nv, ne, eu.data(), ev.data() };   ── 组装只读视图
④ sc_graph_xxx(&g, out...)  ──▶  把整数结果翻译回领域（报错定位 / 烘焙常量 / 环成员 id）
```

算法本体（DFS/Tarjan/Kahn…）**只存在于 `graph_impl.c` 一处**；编译器与运行时都只做①②④的「适配」，
从而「一处修算法、两端同步生效」。

### 12.5 编译器实现落点

- 契约与实现：`builtins/graph/graph.h`（spec 接口）+ `graph_impl.c`（唯一实现）+ `graph.sc`（sc 侧声明）
- 构建接线：`compiler/CMakeLists.txt` 把 `graph_impl.c` 编入 scc；`target_include_directories(... builtins)` 启用 `#include "graph/graph.h"`
- 场景一：`semantic.cpp::checkTokDepGraphCycles`（`sc_graph_cycle`）+ tok 图度量烘焙（`depth/critical/scc/…`）
- 场景二：`proggraph.cpp::findCycles`（`sc_graph_scc`）
