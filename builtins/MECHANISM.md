...
> **机制框架**
>
> com 的核心是**协议对象（vtable 方法指针）+ 编译器语法糖展开 + 运行时框架循环**三层分离：
> - **类型层**：`com` 是 `<limit>` 容器结构，`read`/`write`/`readable`/`writable` 均为每对象方法指针（MethodPtr），设备逻辑完全由指针指向的实现决定
> - **编译层**：`<<` / `>>` 只是普通二元运算符，编译器检测到最左操作数是 `com` 类型时识别为通讯链，按「同步/异步 × 右操作数形态」展开为方法调用或状态机切点
> - **运行时层**：`limit_read`（有界读框架循环）+ `ioq`（循环缓冲队列）+ `async_io`（多路复用事件循环），三者构成设备无关的 io 驱动引擎
