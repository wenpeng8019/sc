 ...
 * 默认实现见 mt_impl.c 的 default_pool(n)；run 的入池帧与 thread_run 同形。 */
typedef struct pool {
    void   *h;       /* 实现私有区指针（队列 + 同步原语 + 工作线程，实现私有） */
    uint8_t (*run)(struct pool *_this, void (*fn)(void *), const void *params, size_t psize);
    void    (*join)(struct pool *_this);
