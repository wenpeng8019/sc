...
typedef uint8_t (*bst_each_fn)(const void *key, sc_afat value, void *ctx);    /* 中序遍历回调：返回 0 提前终止 */
...
sc_afat  bst_get(bst *_this, const void *key);        /* 借用视图（不改计数）；未命中返回空句柄 */
uint8_t  bst_put(bst *_this, const void *key, sc_afat value);  /* 插入/替换：retain 新、替换时 release 旧 */
...
sc_afat      bst_value_at(bst *_this, int64_t cur);   /* 游标处 value 借用（无效返回空句柄） */
...
sc_afat      lru_value_at(lru *_this, int64_t cur);   /* 游标处 value 借用（无效返回空句柄） */
