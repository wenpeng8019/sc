...
bool     string_reserve(string *_this, uint64_t n);
...
bool     string_printf(string *_this, const char *fmt, ...);  /* 追加格式化文本（自动扩容） */
...
bool     array_reserve(array *_this, uint64_t n);
bool     array_resize(array *_this, uint64_t n);      /* 调整元素数，新增槽位清零 */
bool     array_assign(array *_this, void *src, uint64_t n);   /* 赋值为 n 个元素 */
bool     array_append(array *_this, void *src, uint64_t n);   /* 追加 n 个元素 */
bool     array_push(array *_this, void *value);       /* 追加单元素（拷贝 elem_sz 字节） */
...
bool     array_slice(array *_this, int64_t start, int64_t stop, array *out);  /* 负索引从尾部计 */
void     array_reverse(array *_this);
bool     array_clone(array *_this, array *out);       /* 深拷贝 */
...
bool     ring_init(ring *_this, uint32_t elem_sz, uint32_t capacity);  /* capacity 向上取 2 幂；分配失败返回 0 */
...
bool     ring_push(ring *_this, void *value);         /* 生产者入队一个元素（拷贝 elem_sz 字节）；满返回 0 */
bool     ring_pop(ring *_this, void *out);            /* 消费者出队到 out（拷贝 elem_sz 字节）；空返回 0 */
...
bool     list_reserve(list *_this, uint64_t n);
bool     list_push(list *_this, sc_thin value);       /* 尾部追加，retain 元素 */
bool     list_pop(list *_this);                       /* 删除并 release 尾元素；空返回 0 */
sc_thin  list_get(list *_this, uint64_t index);       /* 借用视图（不改计数）；越界返回空句柄 */
bool     list_set(list *_this, uint64_t index, sc_thin value);  /* 改写：retain 新、release 旧 */
bool     list_insert(list *_this, uint64_t index, sc_thin value);
bool     list_remove_at(list *_this, uint64_t index); /* 删除并 release 该元素；越界返回 0 */
int64_t  list_index_of(list *_this, sc_thin value);   /* 按 .p 实体基址比对；未找到 -1 */
void     list_reverse(list *_this);
bool     list_clone(list *_this, list *out);          /* 逐元素 retain 到 out */
...
typedef bool    (*dict_each_fn)(const void *key, sc_thin value, void *ctx);  /* 遍历回调：返回 0 提前终止 */
...
bool     dict_has(dict *_this, const void *key);
sc_thin  dict_get(dict *_this, const void *key);      /* 借用视图（不改计数）；未命中返回空句柄 */
bool     dict_put(dict *_this, const void *key, sc_thin value);  /* 插入/替换：retain 新、替换时 release 旧 */
bool     dict_remove(dict *_this, const void *key);   /* 删除并 release value；未命中返回 0 */
...
typedef bool    (*bst_each_fn)(const void *key, sc_thin value, void *ctx);    /* 中序遍历回调：返回 0 提前终止 */
...
bool     bst_put(bst *_this, const void *key, sc_thin value);  /* 插入/替换：retain 新、替换时 release 旧 */
bool     bst_remove(bst *_this, const void *key);     /* 删除并 release value；未命中返回 0 */
...
bool     heap_is_empty(heap *_this);
void     heap_clear(heap *_this);                     /* 清空并 release 全部 value（保留容量） */
bool     heap_reserve(heap *_this, uint64_t n);       /* 预留至少 n 槽 */
bool     heap_push(heap *_this, const void *key, sc_thin value);  /* 入堆：retain value，上滤；失败 0 */
bool     heap_pop(heap *_this);                       /* 弹出堆顶并 release，下滤；空返回 0 */
...
typedef bool    (*trie_each_fn)(const char *key, sc_thin value, void *ctx);  /* 遍历回调：完整键串；返回 0 提前终止 */
...
bool     trie_has(trie *_this, const char *key);       /* 是否含精确键 */
sc_thin  trie_get(trie *_this, const char *key);       /* 借用键对应 value（未命中空句柄；不改计数） */
bool     trie_put(trie *_this, const char *key, sc_thin value);  /* 插入/替换：retain 新、替换 release 旧 */
bool     trie_remove(trie *_this, const char *key);    /* 删除并 release；未命中 0；剪枝空节点 */
void     trie_clear(trie *_this);                      /* 清空并 release 全部 value */
bool     trie_has_prefix(trie *_this, const char *prefix);   /* 是否存在以 prefix 开头的键 */
...
typedef bool    (*lru_each_fn)(const void *key, sc_thin value, void *ctx);  /* 遍历回调（MRU→LRU）：返回 0 提前终止 */
...
bool     lru_is_empty(lru *_this);
uint64_t lru_cap(lru *_this);                         /* 当前容量上限（0 = 无界） */
void     lru_set_cap(lru *_this, uint64_t cap);       /* 调整容量（缩容立即淘汰 LRU 至 len<=cap） */
bool     lru_has(lru *_this, const void *key);        /* 是否含键（不触顶） */
sc_thin  lru_get(lru *_this, const void *key);        /* 取值并触顶（移至 MRU）；未命中空句柄；借用 */
sc_thin  lru_peek(lru *_this, const void *key);       /* 取值不触顶；未命中空句柄；借用 */
bool     lru_put(lru *_this, const void *key, sc_thin value);  /* 插入/替换 + 触顶：retain 新、替换 release 旧；超容淘汰 */
bool     lru_remove(lru *_this, const void *key);     /* 删除并 release value；未命中返回 0 */
