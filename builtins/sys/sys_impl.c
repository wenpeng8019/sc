...
int32_t sys_work_dir(char *buf, uint32_t size) {
    if (!buf || size == 0) return SYS_ERR_CAPACITY;
#if P_WIN
    /* 成功：返回写入字符数(<size)；buffer 不足：返回所需大小(>=size)；失败：0 */
    DWORD len = GetCurrentDirectoryA(size, buf);
    if (len == 0) return SYS_ERR;
    if (len >= size) return SYS_ERR_CAPACITY;
#else
    if (!getcwd(buf, size)) return (errno == ERANGE) ? SYS_ERR_CAPACITY : SYS_ERR;
#endif
    return SYS_OK;
}
...
int32_t sys_home_dir(char *buf, uint32_t size) {
    ...
    if (FAILED(hr)) { if (wpath) CoTaskMemFree(wpath); return SYS_ERR; }
    int len = WideCharToMultiByte(CP_UTF8, 0, wpath, -1, buf, (int)size, NULL, NULL);
    CoTaskMemFree(wpath);
    if (len == 0) return SYS_ERR_CAPACITY;
#else
    ...
    /* 回退：线程安全的 getpwuid_r */
    {
        ...
        if (getpwuid_r(getuid(), &pw, tmp, sizeof(tmp), &res) != 0 ||
            !res || !pw.pw_dir || !pw.pw_dir[0]) return SYS_ERR;
...
int32_t sys_download_dir(char *buf, uint32_t size) {
    ...
    if (st == 0) {
        /* 回退 ~/Downloads */
        int32_t r = sys_home_dir(buf, size);
        if (r != SYS_OK) return r;
        size_t len = strlen(buf);
        if (len + sizeof("/Downloads") > size) return SYS_ERR_CAPACITY;
    ...
    if (path[0] == '~') {
        char home[PATH_MAX];
        int32_t r = sys_home_dir(home, sizeof(home));
        if (r != SYS_OK) return r;
        size_t hl = strlen(home), rl = strlen(path + 1);  /* 跳过 ~ */
        if (hl + rl >= size) return SYS_ERR_CAPACITY;
        memcpy(buf, home, hl);
...
#if P_WIN
    char *path = NULL;
    if (_get_pgmptr(&path) != 0 || !path) { buf[0] = 0; return SYS_ERR; }
    size_t len = strlen(path);
    if (len >= size) { buf[0] = 0; return SYS_ERR_CAPACITY; }
    memcpy(buf, path, len + 1);
#elif P_DARWIN
    uint32_t bsize = size;
    /* 失败即 buffer 不足：_NSGetExecutablePath 把所需大小写回 bsize */
    if (_NSGetExecutablePath(buf, &bsize) != 0) { buf[0] = 0; return SYS_ERR_CAPACITY; }
    char *canon = realpath(buf, NULL);
    if (!canon) { buf[0] = 0; return SYS_ERR; }
    size_t len = strlen(canon);
    if (len >= size) { free(canon); buf[0] = 0; return SYS_ERR_CAPACITY; }
    memcpy(buf, canon, len + 1);
    free(canon);
#elif P_BSD
    int mib[4] = { CTL_KERN, KERN_PROC, KERN_PROC_PATHNAME, -1 };
    size_t blen = size;
    if (sysctl(mib, 4, buf, &blen, NULL, 0) != 0) { buf[0] = 0; return SYS_ERR; }
    if (blen >= size) { buf[0] = 0; return SYS_ERR_CAPACITY; }
    buf[blen] = 0;   /* 保证 NUL 结尾（sysctl 长度可能不含终止符） */
#elif P_LINUX
    ssize_t len = readlink("/proc/self/exe", buf, size);
    if (len < 0) { buf[0] = 0; return SYS_ERR; }
    if ((uint32_t)len >= size) { buf[0] = 0; return SYS_ERR_CAPACITY; }
    buf[len] = 0;    /* readlink 不补 NUL */
#else
    buf[0] = 0;
    return SYS_ERR;  /* 未知平台：优雅失败而非编译中断 */
#endif
    return SYS_OK;
}
...
int32_t sys_tmp_file(char *buf, uint32_t size) {
    ...
    if (dlen == 0 || dlen >= sizeof(dir)) return SYS_ERR;
    if (size < MAX_PATH) return SYS_ERR_CAPACITY;   /* GetTempFileNameA 要求 buf >= MAX_PATH */
    /* 原子创建唯一空文件并写回完整路径 */
    if (GetTempFileNameA(dir, "sc", 0, buf) == 0) return SYS_ERR;
#else
    ...
    if (n < 0 || (uint32_t)n >= size) return SYS_ERR_CAPACITY;
    int fd = mkstemp(buf);   /* 原子创建唯一空文件 */
    if (fd < 0) return SYS_ERR;
    close(fd);
#endif
    return SYS_OK;
}
