...
#define SSL_BACKEND_NONE    0
#define SSL_BACKEND_MBEDTLS 1
#define SSL_BACKEND_OPENSSL 2

/* 握手 / 读写返回码。
 * 注意：ssl_read/ssl_write 用 >0 表示明文字节数，故 SSL_WANT_IO 必须为负，
 * 否则会与「读到 1 字节」的合法返回值冲突（曾导致首字节读出垃圾）。 */
#define SSL_OK       0    /* 完成 / 干净关闭（读返回 0） */
#define SSL_WANT_IO (-2)  /* 需更多 io：握手未完 / 读写需在传输就绪后重试 */
#define SSL_ERR    (-1)   /* 错误 */

/* 不透明连接句柄：实体定义在 ssl_impl.c（随后端而异）。
 * 注意——公开 ABI 一律用 void* 而非 ssl_conn*，以与 scc 按 ssl.sc 生成的 @fnc
...
typedef int (*ssl_send_fn)(void *ctx, const void *buf, uint32_t len);
typedef int (*ssl_recv_fn)(void *ctx, void *buf, uint32_t len);

/* ---------------- 后端探测 ---------------- */
int32_t  ssl_backend(void);        /* SSL_BACKEND_* */
char    *ssl_backend_name(void);   /* "none" / "mbedtls" / "openssl" */

/* ---------------- 连接生命周期 ---------------- */
/* 新建 TLS 客户端连接（未握手）；host=SNI/校验域名；verify!=0 校验对端证书。失败返回 NULL。 */
void    *ssl_client_new(char *host, int32_t verify);
/* 注入传输回调（send/recv 实为 ssl_send_fn/ssl_recv_fn，及透传 ctx）。 */
void     ssl_set_transport(void *s, void *send, void *recv, void *ctx);
/* 释放连接及底层 TLS 上下文（NULL 安全）。 */
void     ssl_free(void *s);

/* ---------------- 握手与明文收发 ---------------- */
/* 驱动握手：SSL_OK 完成 / SSL_WANT_IO 需再 io / SSL_ERR 失败。 */
int32_t ssl_handshake(void *s);
/* 明文读（自动 TLS 解密）：返回 >=0 字节 / SSL_WANT_IO / SSL_ERR。 */
int32_t ssl_read (void *s, void *buf, uint32_t len);
/* 明文写（自动 TLS 加密）：返回 >=0 字节 / SSL_WANT_IO / SSL_ERR。 */
int32_t ssl_write(void *s, void *buf, uint32_t len);

/* ---------------- com 设备适配层（后端无关，仅用公开 ssl_* API） ----------------
...
struct com;   /* op.h 定义；此处仅作返回/参数类型的不完全声明 */
