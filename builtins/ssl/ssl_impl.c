...
int32_t  sc_ssl_backend(void)      { return SC_SSL_BACKEND_MBEDTLS; }
char    *sc_ssl_backend_name(void) { return (char *)"mbedtls"; }
...
static int mbed_bio_send(void *ctx, const unsigned char *buf, size_t len) {
    struct sc_ssl_conn *s = (struct sc_ssl_conn *)ctx;
...
void *sc_ssl_client_new(char *host, int32_t verify) {
    struct sc_ssl_conn *s = (struct sc_ssl_conn *)calloc(1, sizeof *s);
    if (!s) return NULL;
...
void sc_ssl_set_transport(void *sv, void *send, void *recv, void *ctx) {
    struct sc_ssl_conn *s = (struct sc_ssl_conn *)sv;
    if (!s) return;
    s->send = (sc_ssl_send_fn)send;
    s->recv = (sc_ssl_recv_fn)recv;
    s->tctx = ctx;
    mbedtls_ssl_set_bio(&s->ssl, s, mbed_bio_send, mbed_bio_recv, NULL);
}

void sc_ssl_free(void *sv) {
    struct sc_ssl_conn *s = (struct sc_ssl_conn *)sv;
    if (!s) return;
...
int32_t sc_ssl_handshake(void *sv) {
    struct sc_ssl_conn *s = (struct sc_ssl_conn *)sv;
    if (!s) return SC_SSL_ERR;
    int r = mbedtls_ssl_handshake(&s->ssl);
    if (r == 0) return SC_SSL_OK;
    if (r == MBEDTLS_ERR_SSL_WANT_READ || r == MBEDTLS_ERR_SSL_WANT_WRITE)
        return SC_SSL_WANT_IO;
    return SC_SSL_ERR;
}

int32_t sc_ssl_read(void *sv, void *buf, uint32_t len) {
    struct sc_ssl_conn *s = (struct sc_ssl_conn *)sv;
    if (!s) return SC_SSL_ERR;
...
int32_t sc_ssl_write(void *sv, void *buf, uint32_t len) {
    struct sc_ssl_conn *s = (struct sc_ssl_conn *)sv;
    if (!s) return SC_SSL_ERR;
...
int32_t crypto_rsa_backend(void) { return SC_SSL_BACKEND_MBEDTLS; }
...
int32_t  sc_ssl_backend(void)      { return SC_SSL_BACKEND_OPENSSL; }
char    *sc_ssl_backend_name(void) { return (char *)"openssl"; }

/* 把 wbio 里 SSL 写出的密文全部经 send 发到底层。SC_SSL_OK / SC_SSL_WANT_IO（传输忙）/ SC_SSL_ERR。 */
static int ssl_flush_out(struct ssl_conn *s) {
    ...
    while ((n = BIO_read(s->wbio, buf, (int)sizeof buf)) > 0) {
        ...
        while (off < n) {
            ...
            if (w < 0) return SC_SSL_ERR;
            if (w == 0) return SC_SSL_WANT_IO;   /* 传输暂不可写：剩余下次再发 */
            off += w;
        }
    }
    return SC_SSL_OK;
...
/* 从底层经 recv 取一批密文喂进 rbio。SC_SSL_OK（有数据）/ SC_SSL_WANT_IO（暂无）/ SC_SSL_ERR。 */
static int ssl_pump_in(struct ssl_conn *s) {
    unsigned char buf[4096];
    int n = s->recv(s->tctx, buf, (int)sizeof buf);
    if (n < 0) return SC_SSL_ERR;
    if (n == 0) return SC_SSL_WANT_IO;   /* 暂无数据 / EOF */
...
int32_t sc_ssl_handshake(void *sv) {
    struct ssl_conn *s = (struct ssl_conn *)sv;
    if (!s || !s->send || !s->recv) return SC_SSL_ERR;
    int r = SSL_do_handshake(s->ssl);
    if (ssl_flush_out(s) == SC_SSL_ERR) return SC_SSL_ERR;   /* 先把握手报文发出 */
    if (r == 1) return SC_SSL_OK;
    int err = SSL_get_error(s->ssl, r);
    if (err == SSL_ERROR_WANT_READ) {
        if (ssl_pump_in(s) == SC_SSL_ERR) return SC_SSL_ERR;
        return SC_SSL_WANT_IO;   /* 调用方在传输就绪后再驱动 */
    }
    if (err == SSL_ERROR_WANT_WRITE) return SC_SSL_WANT_IO;
    return SC_SSL_ERR;
}
...
int32_t sc_ssl_read(void *sv, void *buf, uint32_t len) {
    ...
    if (err == SSL_ERROR_WANT_READ) {
        int p = ssl_pump_in(s);
        if (p == SC_SSL_ERR) return SC_SSL_ERR;
        return SC_SSL_WANT_IO;   /* 已喂入新密文（或暂无）：调用方再调一次 */
    }
    if (err == SSL_ERROR_WANT_WRITE) {
        if (ssl_flush_out(s) == SC_SSL_ERR) return SC_SSL_ERR;
        return SC_SSL_WANT_IO;   /* 重协商写出 */
    }
...
int32_t crypto_rsa_backend(void) { return SC_SSL_BACKEND_OPENSSL; }
...
struct sc_ssl_conn { int _unused; };

int32_t  sc_ssl_backend(void)      { return SC_SSL_BACKEND_NONE; }
char    *sc_ssl_backend_name(void) { return (char *)"none"; }
...
void sc_ssl_free(void *s) { (void)s; }

int32_t sc_ssl_handshake(void *s)                          { (void)s; return SC_SSL_ERR; }
int32_t sc_ssl_read (void *s, void *buf, uint32_t len)     { (void)s; (void)buf; (void)len; return SC_SSL_ERR; }
int32_t sc_ssl_write(void *s, void *buf, uint32_t len)     { (void)s; (void)buf; (void)len; return SC_SSL_ERR; }
 ...
 * 要求底层为「阻塞」传输（tcp(fd,false,1,1)）——故 ssl_read/write 的 SC_SSL_WANT_IO 在循环内
 * 由阻塞传输的下一次读/写解除。异步（非阻塞 + async_loop）形态留待后续。
 */
#include "op.h"   /* com / limit / ioq / sc_again / sc_eof / sc_chunk0 / sc_recycle */

typedef struct sc_ssl_dev {
    sc_com      com;            /* 端点（offset 0，返回其地址即 com&） */
    void     *ssl;            /* sc_ssl_conn*（不透明，经公开 API 操作） */
    sc_com      *transport;      /* 底层传输 com（不拥有） */
...
static int sc_ssl_tx_recv(void *ctx, void *buf, uint32_t len) {
    ...
    if (r < 0) { d->tx_err = 1; return -1; }
    if (r == sc_eof)  { d->tx_eof = 1; return 0; }
    if (r == sc_again) return 0;     /* 暂无数据 → ssl 视作 WANT_IO */
    return (int)n;
}

/* com[...] 句柄：limit 缓冲紧随 limit 结构之后分配 */
static void *sc_ssl_limit_data(sc_limit *s) { return (char *)s + sizeof(sc_limit); }
static sc_limit *sc_ssl_alloc(sc_com *_this, uint32_t size, void *ending) {
    (void)_this;
...
static void sc_ssl_free_limit(sc_com *_this, sc_limit *s) { (void)_this; sc_recycle(s); }

/* 设备读：经 TLS 解密读至多 *size 字节，回写实读数。阻塞传输下 SC_SSL_WANT_IO 在循环内解除。
 *   有明文 → 0 / 对端干净关闭或传输 EOF → sc_eof / 错误 → <0。 */
static int32_t sc_ssl_com_read(sc_com *_this, void *data, uint32_t *size) {
    ...
    *size = 0;
    for (;;) {
        d->tx_err = 0;
        int r = sc_ssl_read(d->ssl, data, want);
        if (r > 0) { *size = (uint32_t)r; return 0; }
        if (r == 0) return sc_eof;                 /* TLS close_notify */
        if (r == SC_SSL_WANT_IO) {
            if (d->tx_err) return -1;
            if (d->tx_eof) return sc_eof;          /* 传输层 EOF 且无更多明文 */
...
static int32_t sc_ssl_com_write(sc_com *_this, void *buf, uint32_t *size) {
    sc_ssl_dev *d = (sc_ssl_dev *)_this;
    if (!size) return -1;
    uint32_t want = *size;
    *size = 0;
    for (;;) {
        d->tx_err = 0;
        int r = sc_ssl_write(d->ssl, buf, want);
        if (r > 0) { *size = (uint32_t)r; return 0; }
        if (r == SC_SSL_WANT_IO) {
            if (d->tx_err) return -1;
            if (d->tx_eof) return sc_eof;
            continue;
        }
        return -1;
    }
...
static int32_t sc_ssl_com_error(sc_com *_this) {
    sc_ssl_dev *d = (sc_ssl_dev *)_this;
    return d->tx_err ? -1 : 0;
}
...
static int32_t sc_ssl_readable(sc_com *_this, void **id) {
    sc_ssl_dev *d = (sc_ssl_dev *)_this;
...
static int32_t sc_ssl_writable(sc_com *_this, void **id) {
    sc_ssl_dev *d = (sc_ssl_dev *)_this;
...
static int32_t sc_ssl_com_close(sc_com *_this) {
    sc_ssl_dev *d = (sc_ssl_dev *)_this;
    int32_t r = 0;
    if (d->ssl) { sc_ssl_free(d->ssl); d->ssl = NULL; }
...
struct sc_com *ssl_com(struct sc_com *transport, char *host, int32_t verify) {
    ...
    if (!d) { sc_ssl_free(s); return NULL; }
    d->ssl       = s;
    d->transport = transport;
    sc_ssl_set_transport(s, (void *)sc_ssl_tx_send, (void *)sc_ssl_tx_recv, d);

    /* 同步驱动握手（阻塞传输：循环至完成/失败） */
    int r;
    while ((r = sc_ssl_handshake(s)) == SC_SSL_WANT_IO) {
        if (d->tx_err || d->tx_eof) break;
    }
    if (r != SC_SSL_OK) { sc_ssl_free(s); sc_recycle(d); return NULL; }
