...
#include "platform.h"   /* sc_mutex_* / sc_thread_id（全局可重入锁）；末尾带入 op.h（sc_thin/SC_OWN_RAW） */
#include "adt/adt.h"    /* dict：g_toks 哈希表（裸 token* 经 sc_thin 存取） */
...
static sc_thin tok_afat(token *t) {
    sc_thin f;
...
static sc_thin tok_run_combine(token *t, sc_thin base, sc_thin input, int32_t tag) {
    __sctok_in self;
...
static sc_thin tok_value_excl(token *t) {
    sc_thin v;
    v.p    = sc_get_ord(&t->value.p);
    v.tar  = sc_get_ord(&t->value.tar);
    v.dtor = sc_get_ord(&t->value.dtor);
...
static void tok_pending_push(token *t, sc_thin v, int32_t tag) {
...
static void tok_set_impl(token *t, sc_thin v, int32_t tag, int force) {
    if (!t) return;
...
void token_set(token *t, sc_thin v, int32_t tag) {
    tok_set_impl(t, v, tag, 0);                        /* 记忆化：同值抑制（默认） */
}

void token_pulse(token *t, sc_thin v, int32_t tag) {
    tok_set_impl(t, v, tag, 1);                        /* 脉冲：绕过相等抑制，同值也强制传播（拉取流水线/迭代） */
}

void token_form(token *t, sc_thin v, int32_t tag, void *ctx, token_exec exec) {
    if (!t) return;
 ...
 *   行为不变（向后兼容）。 */
void token_back(token *t, sc_thin seed, int32_t tag) {
    if (!t) return;
