...
scc app.sc --target examples/targets/windows-x64.target 2>/dev/null | LC_ALL=C tr -d '\r'
```
...
MSVC 远端编译配方要点（见 [examples/targets/windows-x64.target](examples/targets/windows-x64.target)）：
