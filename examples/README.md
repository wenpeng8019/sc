...
| feature10.sc | `prev`/`next` 上下文关键字（链表结构体成员访问位）；io 子模块：`print` 日志输出（F/E/W/I/D/V 级别、SC_LOG 过滤）与 `stringify(值[, 缓存, 大小])` JSON 格式化（标量/结构体/数组/枚举→string 或缓存 char&；结构体指针成员→`"类型名@0x地址"`，标量指针成员→`"&值"`；选项块 `stringify<compact:1>(值)` 紧凑单行，默认多行美化） |
