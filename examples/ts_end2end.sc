...
fnc main: i4
    ...
    shp[0] = 2
    shp[1] = 3
    var x: tensor@& = arange(0.0, 6.0, 1.0, DT_F8)
    var x2: tensor@& = x->reshape(2, shp)

    # 2) 统计与标准化
    var mu: f8 = x2->mean_all()
    var sd: f8 = x2->std_all()
    var xn: tensor@& = x2->sub_scalar(mu)->div_scalar(sd)
    ...
    var b: tensor@& = zeros(1, s22, DT_F8)
    b->set_at(0, 1.0)
    b->set_at(1, 2.0)

    var Minv: tensor@& = M->inv()
    var sol: tensor@& = M->solve(b)
    printf("linalg det=%g inv00=%.4f x0=%.4f x1=%.4f\n", M->det(), Minv->at(0), sol->at(0), sol->at(1))

    # 4) meshgrid（ij）
    var xv: tensor@& = arange(0.0, 3.0, 1.0, DT_F8)
    var yv: tensor@& = arange(0.0, 2.0, 1.0, DT_F8)
    var inp[2]: &
    inp[0] = (xv: &)
    inp[1] = (yv: &)
    var out[2]: &
    ...
    var xl: tensor@& = ts_load("/tmp/ts_end2end.npy")
