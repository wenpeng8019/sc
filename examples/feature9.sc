...
#   - def T: ~ {}：链表标记，转 C 在成员末尾注入 T *_prev / T *_next
#   - chain（inc adt.sc）：head 为首元素；首元素 _prev = 尾元素（rear），尾元素 _next = nil
#   - append/push 时编译器自动注入元素 _prev 偏移（同一 chain 只存同一种结构体）
