# 特性 41：async ..., q —— 非阻塞带回复的队列投递（rpc 作流原语的第五种驱动）
#
#   - 带队列 `async work(args), q`：把 rpc 调用【非阻塞】投递给队列 q，立即返回
#     promise&（mt-future 句柄）；某消费者（另一线程 pull / 线程池工作线程）执行完成
#     后兑现该 promise，调用方经 p->wait() 阻塞取回结果（或 p->ready() 非阻塞轮询）。
#   - 与 sync ..., q（特性 40）对照：sync 立即阻塞等回复；async 先拿 promise，可并发
...
#   - 逗号紧绑 async：`var p = async work(a, b), q`。超时 / 优先级 / 取消为后续阶段。
#
inc mt.sc
...
fnc main: i4
    ...
    var f2: promise& = async compute(100, 23), q
    ...
    var g1: promise& = async compute(10, 20), q2     # 非阻塞投递
    var g2: promise& = async compute(5, 6), q2
