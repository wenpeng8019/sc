# shader-tri-metal —— syntax-s 发行路径（SPIR-V → MSL）

与 [`../shader-tri`](../shader-tri) 是同一个三角形着色器（共用其 `tri.ss`），但走**发行侧的离线转译链**，直接产出**原生 Metal** 着色库，而不是靠 MoltenVK 在运行时翻译 Vulkan。
...
```
scc tri.ss  → vs_main.vert / fs_main.frag   (Vulkan-GLSL)
...
SPIR-V 作为中枢 IR：`tri.ss` 声明 `tar vulkan@450`，先出 Vulkan-GLSL 再编成 SPIR-V，
...
- **A（本例，已完成）**：打通 `.ss → … → .metallib` 发行链，验证 MSL 合法且可编译。
