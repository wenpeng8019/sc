...
| `i4` | 32 位有符号整数 | `i1`/`i2` 可写，按 32 位处理 |
| `u4` | 32 位无符号整数 | `u1`/`u2` 同上；GLES100/WebGL1 无 uint（能力门控） |
| `bool` | 布尔 | |
...
comp cs_main: void, in: CompIn     # 计算：返回 void，入参计算内建结构
...
`comp` 的工作组尺寸当前固定 `local_size = 64×1×1`（写入反射清单，运行时据此调度）。
...
需 gles≥300；`frag_depth` 在 gles100 经 `GL_EXT_frag_depth`；等（单一事实源
`shader_caps.h` 能力表）。
