...
@fnc sa_llm_body: out: string&, model: const char&, sys_msg: const char&, user_msg: const char&, stream: bool
    out->append("{\"model\":")
...
fnc sa_llm_stream_exec: i4, endpoint: const char&, bearer: const char&, body: const char&, timeout_s: i4, answer: string&
    var fp: & = sa_http_sse_open(endpoint, bearer, body, timeout_s)
    if fp == nil
    ...
    while ::fgets((buf: &), 8192, fp) != nil
        ...
        if buf[0] == (100: char) && buf[1] == (97: char) && buf[2] == (116: char) && buf[3] == (97: char) && buf[4] == (58: char)
            ...
            if sa_json_get_str((buf + p: const char&), "content", chunk) == 0 && chunk->len() > 0
                ::printf("%s", chunk->cstr())
                ::fflush(::stdout)
    ...
    if answer->len() == 0
        ...
        if errbuf->len() > 0 && sa_json_get_str(errbuf->cstr(), "message", em) == 0
...
@fnc sa_llm_request_ex: i4, cfg: sa_cfg&, sect: const char&, sys_msg: const char&, user_msg: const char&, answer: string&, stream: bool
    ...
    sa_llm_body(body, model, sys_msg, user_msg, stream)
    if stream
        ::system("rm -f .sagent/tmp/resp.json")   # 防旧响应被误归档
        var src: i4 = sa_llm_stream_exec(endpoint, bearer, body->cstr(), timeout_s, answer)
        body->drop()
        if src == 0 && asked
