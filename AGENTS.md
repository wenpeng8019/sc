...
| spc | ✅ Metal kernel + MPSGraph + CoreML/ANE(86% 实测)；P2/P3 计算语法全链实测；spec 传值实测 | kernel 后端已盲写(vulkan_spc/gl_spc,交叉编译过)，板验按 spc/PORTING.md | vulkan_spc 已盲写(mingw 编译过) |
...
- comp 已通(计算内建映射如 `global_invocation_id`,标量声明自动 `.x`;
  **P2 计算深化已落地(2026-07-13)**:签名尾 `local X [Y [Z]]` 工作组尺寸(写反射,
  默认 64×1×1)、`@def X: {...} shared` 共享内存、barrier()/memory_barrier()、
  atomic_* 九函数族(SSBO/shared 的 i4/u4)、`let X: T = v spec N` 特化常量(反射
  spec_constants + 运行时传值 kernel_desc.spec_values)、subgroup 基础三件
  (vote/ballot/shuffle,用到才升 SPIR-V 1.3)。
  **P3 窄/宽标量已落地(同日)**:名字即字节数 f2(f16)/i8·u8(int64)/i1·u1(int8)/
  i2·u2(int16) 真位宽,块成员经 SPV_KHR_16bit/8bit_storage。
  能力门控全入 shader_caps.h;Metal 端到端验证 templates/demo/spc_p2_demo.sc)。
...
5. spc 二期：**Vulkan/GLES31 kernel 后端已盲写完成（vtable + spec 传值，交叉编译
   验证），板端验证照 builtins/spc/PORTING.md**；RKNN model 后端、nn 热点算子接
   graph 面待做。P2/P3 语法侧全部完成（见 syntax-s-design §16）。
6. **CPU/DSP 后端（`tar cpu@99`，M1+M2 已完成实测）**：codegen_cpu 发 SPMD 循环 C +
   spc cpu vtable（强制 CPU 可对拍）；M3 DSP 板落地待触发（syntax-s-design §17.6）。
7. **spc graph 面算子（nn 加速，已立项）**：.ss kernel 算子打底（四后端通用）+
   MPSGraph 快路径保留；路线图 syntax-s-design §18（M1 matmul 通用化进行中 →
   M2 conv/softmax/elementwise + nn 接线 → M3 量化/融合）。
