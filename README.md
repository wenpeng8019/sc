...
语言定义见 [syntax](syntax)，GPU 空间计算方言（`.ss`）语法参考见 [syntax-s.md](syntax-s.md)（设计与路线见 [syntax-s-design.md](syntax-s-design.md)），编译器参考手册见 [compiler.md](compiler.md)（含交叉编译、远程构建、builtins 集成与编译器默认行为），VS Code 调试配置见 [debugging.md](debugging.md)，交叉/远程构建踩坑与解法见 [troubleshooting.md](troubleshooting.md)。
...
./build/scc app.sc --build -o app --target ../templates/.scenv/targets/aarch64-linux.target
./build/scc fw.sc  --build -o fw.bin --target ../templates/.scenv/targets/cortex-m4.target \
...
- io 内置关键字：`print<E>("%d", n)` C 风格日志输出（通道即级别 `F/E/W/I/D/V=1..6`，`print(...)` 无 `<>` 为通道 0 纯 stdout；终端下按级别着色、`SC_LOG` 过滤、`SC_LOG_SYS=1` 镜像系统日志；始终链接无需 inc）；`stringify(值)` JSON 格式化关键字（默认多行美化，返回 `string` 用毕 `drop`；`stringify(值, 缓存, 大小)` 返回缓存 `char&`；选项块 `stringify<compact:1>(值)` 紧凑单行；需 `inc adt.sc` + `inc io.sc`，转 C 时生成独立 `stringify.h`）
